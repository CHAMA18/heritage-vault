// HeritageAtlas — Navigation system (bundled by Vite)
...
function initScreen(view: string): void {
  ...
  try {
    if (view === "vault") { const s = document.getElementById("vault-screen"); if (s) initVaultDashboard(s); }
      ...
      const s = document.getElementById("agent-screen");
      if (s) { const c = new AgentChat({ mount: s, dataset: demoAtlasDataset, vaultId: "demo-vault" }); c.init(); }
    }
    initState[view] = true;
...
document.addEventListener("click", (e: Event) => {
  const link = (e.target as HTMLElement).closest<HTMLElement>("[data-dashboard-view]");
  if (!link) return;
  e.preventDefault();
  e.stopPropagation();
  navigateToView(link.dataset.dashboardView || "");
});
...
if (window.location.hash) {
  ...
  if (["vault", "family-map", "story-mode", "agent", "atlas"].includes(h)) { setTimeout(() => navigateToView(h), 200); }
}
