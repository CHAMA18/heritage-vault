...
html.dark .hv-unified-sidebar{background:linear-gradient(180deg,#292622 0%,#211d19 100%);border-color:rgba(231,189,177,.12)}
  ...
  `;const s=n.querySelector("[data-sm-narrative]"),i=n.querySelector("[data-sm-evidence]"),a=n.querySelector("[data-sm-chapter-label]"),l=n.querySelector("[data-sm-prev]"),c=n.querySelector("[data-sm-next]");if(!s||!i||!a||!l||!c)return;function u(V){return ct.memories.find(F=>F.id===V)}function d(V,F){const j=e[V];if(!j)return;t=V,r.add(j.id),a.textContent=`Chapter ${j.num} · ${j.label}`,n.querySelectorAll("[data-sm-step]").forEach(K=>{var g;const T=parseInt(K.dataset.smStep??"0",10);K.classList.toggle("is-active",T===V),K.classList.toggle("is-read",T<V||r.has(((g=e[T])==null?void 0:g.id)??"")&&T!==V)}),l.disabled=V===0,c.disabled=V===e.length-1;const q=s;F?(q.classList.add("is-transitioning"),setTimeout(()=>{q.classList.remove("is-transitioning"),q.classList.add("is-entering"),q.innerHTML=p(j),v(j),w(q),b(q),q.offsetWidth,q.classList.remove("is-entering"),q.scrollTop=0,window.scrollTo({top:n.offsetTop-24,behavior:"smooth"})},280)):(q.innerHTML=p(j),v(j),w(q),b(q))}function p(V){const F=V.memoryIds.map(u).filter(Boolean),j=F.find(g=>(g==null?void 0:g.type)==="photo")??F[0],q=V.excerpt,K=V.body.split(new RegExp("(?<=[.!?])\\s+")),T=[];if(K.length<=2)T.push(V.body);else{const g=K.slice(0,Math.ceil(K.length/3)).join(" "),A=K.slice(Math.ceil(K.length/3),Math.ceil(2*K.length/3)).join(" "),I=K.slice(Math.ceil(2*K.length/3)).join(" ");T.push(g,A,I)}return`
    ...
    `}function v(V){const F=V.memoryIds.map(u).filter(Boolean);i.innerHTML=F.map((j,q)=>`
  ...
  `}function tg(n){const e=n.series??[];if(!e.length)return Nt("No data");const t=e[0].points,r=Math.max(...t.map(x=>x.value),1),s=720,i=320,a=48,l=24,c=24,u=56,d=s-a-l,p=i-c-u,v=d/t.length*.62,w=d/t.length*.38,b=Array.from({length:5},(x,k)=>{const B=Math.round(r/4*k),V=c+p-B/r*p;return{v:B,y:V}});return`
    <svg class="hv-agent__chart" viewBox="0 0 ${s} ${i}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${J(n.title)}">
      ${b.map(x=>`
        ...
        `).join("")}
      ${t.map((x,k)=>{const B=a+k*(v+w)+w/2,V=x.value/r*p,F=c+p-V,j=kt[k%kt.length];return`
            <g class="hv-agent__bar" data-drilldown="${J(x.drilldown)}" data-label="${J(x.label)}" data-value="${J(x.value)}">
              <rect x="${B}" y="${F}" width="${v}" height="${V}" rx="6" ry="6" fill="${j}" opacity="0.9">
              ...
              <text x="${B+v/2}" y="${c+p+22}" text-anchor="middle" font-size="11" fill="#3a3f33">${J(x.label)}</text>
              <text x="${B+v/2}" y="${F-6}" text-anchor="middle" font-size="11" font-weight="600" fill="#1d2018">${J(x.value)}</text>
            </g>
          `}).join("")}
      <line x1="${a}" y1="${c+p}" x2="${s-l}" y2="${c+p}" stroke="rgba(28,32,24,0.18)" stroke-width="1.5" />
    </svg>
  `}function ng(n){const e=n.series??[];if(!e.length)return Nt("No data");const t=e[0].points;if(t.length<2)return Nt("Need at least 2 points");const r=Math.max(...t.map(k=>k.value),1),s=720,i=320,a=48,l=24,c=24,u=56,d=s-a-l,p=i-c-u,v=d/(t.length-1),w=Array.from({length:5},(k,B)=>{const V=Math.round(r/4*B),F=c+p-V/r*p;return{v:V,y:F}}),b=t.map((k,B)=>{const V=a+B*v,F=c+p-k.value/r*p;return`${B===0?"M":"L"} ${V} ${F}`}).join(" "),x=`${b} L ${a+(t.length-1)*v} ${c+p} L ${a} ${c+p} Z`;return`
    <svg class="hv-agent__chart" viewBox="0 0 ${s} ${i}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${J(n.title)}">
      <defs>
      ...
      ${w.map(k=>`
        ...
        `).join("")}
      <path d="${x}" fill="url(#hv-agent-line-grad)" />
      ...
      ${t.map((k,B)=>{const V=a+B*v,F=c+p-k.value/r*p;return`
  ...
  `}function ig(n){const e=n.nodes??[],t=n.edges??[];if(!e.length)return Nt("No people");const r=Math.max(...e.map(c=>c.weight??0),1),s=50,i=50,a=32;e.forEach((c,u)=>{const d=u/e.length*Math.PI*2-Math.PI/2,p=a-(c.weight??0)/r*10;c._x=s+p*Math.cos(d),c._y=i+p*Math.sin(d)});const l=new Map(e.map(c=>[c.id,c]));return`
    <div class="hv-agent__network">
      <svg class="hv-agent__network-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
        ${t.map(c=>{const u=l.get(c.from),d=l.get(c.to);if(!u||!d)return"";const p=u._x,v=u._y,w=d._x,b=d._y;return`<line x1="${p}" y1="${v}" x2="${w}" y2="${b}" stroke="rgba(28,32,24,0.18)" stroke-width="0.3" />`}).join("")}
      </svg>
    ...
    <main class="hv-fm-main">
      ...
      <section class="hv-fm-canvas" data-fm-canvas>
        <svg class="hv-fm-edges" data-fm-edges viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          ${t.map(b=>{const x=s(b.from),k=s(b.to),B=i.get(x)??[],V=i.get(k)??[],F=B.findIndex(I=>I.id===b.from),j=V.findIndex(I=>I.id===b.to),q=(F+1)/(B.length+1)*100,K=(x+.5)/a.length*100,T=(j+1)/(V.length+1)*100,g=(k+.5)/a.length*100,A=(K+g)/2;return`<path d="M ${q} ${K} C ${q} ${A}, ${T} ${A}, ${T} ${g}" fill="none" stroke="rgba(45,58,46,0.2)" stroke-width="0.3" stroke-dasharray="0.8 0.8" />`}).join("")}
        </svg>

        <div class="hv-fm-generations">
          ${a.map((b,x)=>{const k=i.get(b)??[];return`
              <div class="hv-fm-generation" style="--gen-delay: ${x*100}ms">
                ...
                <div class="hv-fm-generation__row">
                  ${k.map((B,V)=>{const F=l.get(B.id)??0,j=b===0,q=B.birthYear??"?",K=B.deathYear??"present";return`
                      <button class="hv-fm-node${j?" hv-fm-node--root":""}" data-fm-node data-member-id="${Et(B.id)}" style="--node-delay: ${x*100+V*40}ms">
  ...
  `,ef(n);const c=n.querySelector("[data-add-relative]"),u=n.querySelector("[data-invite-family]"),d=n.querySelector("[data-fm-canvas]");function p(b){var k,B,V;if(!d)return;(k=d.querySelector(".hv-fm-inline-form"))==null||k.remove();const x=document.createElement("div");return x.className="hv-fm-inline-form",x.innerHTML=b,d.prepend(x),(B=x.querySelector(".hv-fm-inline-form__close"))==null||B.addEventListener("click",()=>x.remove()),(V=x.querySelector(".hv-fm-inline-form__cancel"))==null||V.addEventListener("click",()=>x.remove()),x}function v(b,x,k){var B;b.innerHTML=`
      <div class="hv-fm-inline-form__success">
        <span class="material-symbols-outlined">check_circle</span>
        <h3>${x}</h3>
        <p>${k}</p>
        <button type="button" class="hv-fm-inline-form__submit">Done</button>
      </div>`,(B=b.querySelector(".hv-fm-inline-form__submit"))==null||B.addEventListener("click",()=>b.remove())}c&&c.addEventListener("click",()=>{var x;const b=p(`
      ...
      `);b&&((x=b.querySelector("form"))==null||x.addEventListener("submit",k=>{k.preventDefault(),v(b,"Relative added","The new family member has been added to your map.")}))}),u&&u.addEventListener("click",()=>{var x;const b=p(`
        <div class="hv-fm-inline-form__card">
          <div class="hv-fm-inline-form__head">
            <h3>Invite family</h3>
            <button type="button" class="hv-fm-inline-form__close" aria-label="Close">
      ...
      `);b&&((x=b.querySelector("form"))==null||x.addEventListener("submit",k=>{k.preventDefault(),v(b,"Invitation sent","Your family member will receive an email invitation to join HeritageAtlas.")}))});const w=n.querySelector("[data-fm-inspector]");n.querySelectorAll("[data-fm-node]").forEach(b=>{b.addEventListener("click",()=>{const x=b.dataset.memberId??"",k=e.find(q=>q.id===x);if(!k||!w)return;n.querySelectorAll("[data-fm-node]").forEach(q=>q.classList.remove("is-selected")),b.classList.add("is-selected");const B=l.get(x)??0,V=e.find(q=>q.id===k.parentId),F=e.filter(q=>q.parentId===x),j=ct.memories.filter(q=>{var K;return(K=q.familyMemberIds)==null?void 0:K.includes(x)});w.innerHTML=`
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Sg="FirebaseError";class Lt extends Error{constructor(e,t,r){super(t),this.code=e,this.customData=r,this.name=Sg,Object.setPrototypeOf(this,Lt.prototype),Error.captureStackTrace&&Error.captureStackTrace(this,kr.prototype.create)}}class kr{constructor(e,t,r){this.service=e,this.serviceName=t,this.errors=r}create(e,...t){const r=t[0]||{},s=`${this.service}/${e}`,i=this.errors[e],a=i?xg(i,r):"Error",l=`${this.serviceName}: ${a} (${s}).`;return new Lt(s,l,r)}}function xg(n,e){return n.replace(Pg,(t,r)=>{const s=e[r];return s!=null?String(s):`<${r}?>`})}const Pg=/\{\$([^}]+)}/g;function kg(n){for(const e in n)if(Object.prototype.hasOwnProperty.call(n,e))return!1;return!0}function Qn(n,e){if(n===e)return!0;const t=Object.keys(n),r=Object.keys(e);for(const s of t){if(!r.includes(s))return!1;const i=n[s],a=e[s];if(H1(i)&&H1(a)){if(!Qn(i,a))return!1}else if(i!==a)return!1}for(const s of r)if(!t.includes(s))return!1;return!0}function H1(n){return n!==null&&typeof n=="object"}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Bl="[DEFAULT]",N0={[Ul]:"fire-core",[i0]:"fire-core-compat",[o0]:"fire-analytics",[a0]:"fire-analytics-compat",[c0]:"fire-app-check",[l0]:"fire-app-check-compat",[u0]:"fire-auth",[h0]:"fire-auth-compat",[d0]:"fire-rtdb",[f0]:"fire-data-connect",[p0]:"fire-rtdb-compat",[m0]:"fire-fn",[g0]:"fire-fn-compat",[y0]:"fire-iid",[v0]:"fire-iid-compat",[_0]:"fire-fcm",[A0]:"fire-fcm-compat",[E0]:"fire-perf",[w0]:"fire-perf-compat",[b0]:"fire-rc",[I0]:"fire-rc-compat",[T0]:"fire-gcs",[R0]:"fire-gcs-compat",[C0]:"fire-fst",[x0]:"fire-fst-compat",[S0]:"fire-vertex","fire-js":"fire-js",[P0]:"fire-js-all"};/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Sf=1e4,xf=`w:${mc}`,Pf="FIS_v2",W0="https://firebaseinstallations.googleapis.com/v1",G0=3600*1e3,K0="installations",Q0="Installations";/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function e7({appConfig:n,heartbeatServiceProvider:e},{fid:t}){const r=Nf(n),s=Lf(n),i=e.getImmediate({optional:!0});if(i){const u=await i.getHeartbeatsHeader();u&&s.append("x-firebase-client",u)}const a={fid:t,authVersion:Pf,appId:n.appId,sdkVersion:xf},l={method:"POST",headers:s,body:JSON.stringify(a)},c=await Vf(()=>fetch(r,l));if(c.ok){const u=await c.json();return{fid:u.fid||t,registrationStatus:2,refreshToken:u.refreshToken,authToken:Df(u.authToken)}}else throw await Of("Create Installation",c)}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function vc(n,e=!1){let t;const r=await ho(n.appConfig,i=>{if(!jf(i))throw br.create("not-registered");const a=i.authToken;if(!e&&_7(a))return i;if(a.requestStatus===1)return t=y7(n,e),i;{if(!navigator.onLine)throw br.create("app-offline");const l=E7(i);return t=v7(n,l),l}});return t?await t:r.authToken}async function y7(n,e){let t=await Z1(n.appConfig);for(;t.authToken.requestStatus===1;)await Mf(100),t=await Z1(n.appConfig);const r=t.authToken;return r.requestStatus===0?vc(n,e):r}function Z1(n){return ho(n,e=>{if(!jf(e))throw br.create("not-registered");const t=e.authToken;return w7(t)?{...e,authToken:{requestStatus:0}}:e})}async function v7(n,e){try{const t=await m7(n,e),r={...e,authToken:t};return await Na(n.appConfig,r),t}catch(t){if(kf(t)&&(t.customData.serverCode===401||t.customData.serverCode===404))await qf(n.appConfig);else{const r={...e,authToken:{requestStatus:0}};await Na(n.appConfig,r)}throw t}}function jf(n){return n!==void 0&&n.registrationStatus===2}function _7(n){return n.requestStatus===2&&!A7(n)}function A7(n){const e=Date.now();return e<n.creationTime||n.creationTime+n.expiresIn<e+G0}function E7(n){const e={requestStatus:1,requestTime:Date.now()};return{...n,authToken:e}}function w7(n){return n.requestStatus===1&&n.requestTime+Sf<Date.now()}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function b7(n){const e=n,{installationEntry:t,registrationPromise:r}=await yc(e);return r?r.catch(console.error):vc(e).catch(console.error),t.fid}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function I7(n,e=!1){const t=n;return await T7(t),(await vc(t,e)).token}async function T7(n){const{registrationPromise:e}=await yc(n);e&&await e}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function R7(n){if(!n||!n.options)throw vl("App Configuration");if(!n.name)throw vl("App Name");const e=["projectId","apiKey","appId"];for(const t of e)if(!n.options[t])throw vl(t);return{appName:n.name,projectId:n.options.projectId,apiKey:n.options.apiKey,appId:n.options.appId}}function vl(n){return br.create("missing-app-config-values",{valueName:n})}/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Da="analytics",k7="firebase_id",N7="origin",D7=60*1e3,O7="https://firebase.googleapis.com/v1alpha/projects/-/apps/{app-id}/webConfig",_c="https://www.googletagmanager.com/gtag/js";/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function y4(){return typeof navigator<"u"&&navigator&&"onLine"in navigator&&typeof navigator.onLine=="boolean"&&(g4()||uc()||"connection"in navigator)?navigator.onLine:!0}function v4(){if(typeof navigator>"u")return null;const n=navigator;return n.languages&&n.languages[0]||n.language||null}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Pi{constructor(e,t){this.shortDelay=e,this.longDelay=t,vn(t>e,"Short delay should be less than long delay!"),this.isMobile=wg()||Tg()}get(){return y4()?this.isMobile?this.longDelay:this.shortDelay:Math.min(5e3,this.shortDelay)}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function wc(n,e){vn(n.emulator,"Emulator should always be set here");const{url:t}=n.emulator;return e?`${t}${e.startsWith("/")?e.slice(1):e}`:t}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function N4(n,e){const t=await Jf(n,{},async()=>{const r=xi({grant_type:"refresh_token",refresh_token:e}).slice(1),{tokenApiHost:s,apiKey:i}=n.config,a=await Zf(n,s,"/v1/token",`key=${i}`),l=await n._getAdditionalHeaders();l["Content-Type"]="application/x-www-form-urlencoded";const c={method:"POST",headers:l,body:r};return n.emulatorConfig&&Nr(n.emulatorConfig.host)&&(c.credentials="include"),Xf.fetch()(a,c)});return{accessToken:t.access_token,expiresIn:t.expires_in,refreshToken:t.refresh_token}}async function D4(n,e){return wn(n,"POST","/v2/accounts:revokeToken",En(n,e))}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const ch=new Map;function un(n){vn(n instanceof Function,"Expected a class definition");let e=ch.get(n);return e?(vn(e instanceof n,"Instance stored in cache mismatched with class"),e):(e=new n,ch.set(n,e),e)}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class vp{constructor(e,t){this.storageRetriever=e,this.type=t}_isAvailable(){try{return this.storage?(this.storage.setItem(Ba,"1"),this.storage.removeItem(Ba),Promise.resolve(!0)):Promise.resolve(!1)}catch{return Promise.resolve(!1)}}_set(e,t){return this.storage.setItem(e,JSON.stringify(t)),Promise.resolve()}_get(e){const t=this.storage.getItem(e);return Promise.resolve(t?JSON.parse(t):null)}_remove(e){return this.storage.removeItem(e),Promise.resolve()}get storage(){return this.storageRetriever()}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Ap extends vp{constructor(){super(()=>window.sessionStorage,"SESSION")}_addListener(e,t){}_removeListener(e,t){}}Ap.type="SESSION";const Ep=Ap;/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class b3{constructor(e){this.target=e,this.handlers=new Set}removeMessageHandler(e){e.messageChannel&&(e.messageChannel.port1.removeEventListener("message",e.onMessage),e.messageChannel.port1.close()),this.handlers.delete(e)}async _send(e,t,r=50){const s=typeof MessageChannel<"u"?new MessageChannel:null;if(!s)throw new Error("connection_unavailable");let i,a;return new Promise((l,c)=>{const u=Cc("",20);s.port1.start();const d=setTimeout(()=>{c(new Error("unsupported_event"))},r);a={messageChannel:s,onMessage(p){const v=p;if(v.data.eventId===u)switch(v.data.status){case"ack":clearTimeout(d),i=setTimeout(()=>{c(new Error("timeout"))},3e3);break;case"done":clearTimeout(i),l(v.data.response);break;default:clearTimeout(d),clearTimeout(i),c(new Error("invalid_response"));break}}},this.handlers.add(a),s.port1.addEventListener("message",a.onMessage),this.target.postMessage({eventType:e,eventId:u,data:t},[s.port2])}).finally(()=>{a&&this.removeMessageHandler(a)})}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const e9=new Pi(3e4,6e4);function Ah(){const n=Xt().___jsl;if(n!=null&&n.H){for(const e of Object.keys(n.H))if(n.H[e].r=n.H[e].r||[],n.H[e].L=n.H[e].L||[],n.H[e].r=[...n.H[e].L],n.CP)for(let t=0;t<n.CP.length;t++)n.CP[t]=null}}function t9(n){return new Promise((e,t)=>{var s,i,a;function r(){Ah(),gapi.load("gapi.iframes",{callback:()=>{e(gapi.iframes.getContext())},ontimeout:()=>{Ah(),t(Yt(n,"network-request-failed"))},timeout:e9.get()})}if((i=(s=Xt().gapi)==null?void 0:s.iframes)!=null&&i.Iframe)e(gapi.iframes.getContext());else if((a=Xt().gapi)!=null&&a.load)r();else{const l=$4("iframefcb");return Xt()[l]=()=>{gapi.load?r():t(Yt(n,"network-request-failed"))},hp(`${j4()}?onload=${l}`).catch(c=>t(c))}}).catch(e=>{throw va=null,e})}let va=null;function n9(n){return va=va||t9(n),va}/**
 * @license
 * Copyright 2020 Google LLC.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const r9=new Pi(5e3,15e3),s9="__/auth/iframe",i9="emulator/auth/iframe",a9={style:{position:"absolute",top:"-100px",width:"1px",height:"1px"},"aria-hidden":"true",tabindex:"-1"},o9=new Map([["identitytoolkit.googleapis.com","p"],["staging-identitytoolkit.sandbox.googleapis.com","s"],["test-identitytoolkit.sandbox.googleapis.com","t"]]);function l9(n){const e=n.config;ne(e.authDomain,n,"auth-domain-config-required");const t=e.emulator?wc(e,i9):`https://${n.config.authDomain}/${s9}`,r={apiKey:e.apiKey,appName:n.name,v:Dr},s=o9.get(n.config.apiHost);s&&(r.eid=s);const i=n._getFrameworks();return i.length&&(r.fw=i.join(",")),`${t}?${xi(r).slice(1)}`}async function c9(n){const e=await n9(n),t=Xt().gapi;return ne(t,n,"internal-error"),e.open({where:document.body,url:l9(n),messageHandlersFilter:t.iframes.CROSS_ORIGIN_IFRAMES_FILTER,attributes:a9,dontclear:!0},r=>new Promise(async(s,i)=>{await r.restyle({setHideOnLeave:!1});const a=Yt(n,"network-request-failed"),l=Xt().setTimeout(()=>{i(a)},r9.get());function c(){Xt().clearTimeout(l),s(r)}r.ping(c).then(c,()=>{i(a)})}))}/**
 * @license
 * Copyright 2020 Google LLC.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const y9="__/auth/handler",v9="emulator/auth/handler",_9=encodeURIComponent("fac");async function wh(n,e,t,r,s,i){ne(n.config.authDomain,n,"auth-domain-config-required"),ne(n.config.apiKey,n,"invalid-api-key");const a={apiKey:n.config.apiKey,appName:n.name,authType:t,redirectUrl:r,v:Dr,eventId:s};if(e instanceof pp){e.setDefaultLanguage(n.languageCode),a.providerId=e.providerId||"",kg(e.getCustomParameters())||(a.customParameters=JSON.stringify(e.getCustomParameters()));for(const[d,p]of Object.entries({}))a[d]=p}if(e instanceof Ni){const d=e.getScopes().filter(p=>p!=="");d.length>0&&(a.scopes=d.join(","))}n.tenantId&&(a.tid=n.tenantId);const l=a;for(const d of Object.keys(l))l[d]===void 0&&delete l[d];const c=await n._getAppCheckToken(),u=c?`#${_9}=${encodeURIComponent(c)}`:"";return`${A9(n)}?${xi(l).slice(1)}${u}`}function A9({config:n}){return n.emulator?wc(n,v9):`https://${n.authDomain}/${y9}`}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const El="webStorageSupport";class E9{constructor(){this.eventManagers={},this.iframes={},this.originValidationPromises={},this._redirectPersistence=Ep,this._completeRedirectFn=z3,this._overrideRedirectResult=H3}async _openPopup(e,t,r,s){var a;vn((a=this.eventManagers[e._key()])==null?void 0:a.manager,"_initialize() not called before _openPopup()");const i=await wh(e,t,r,Hl(),s);return m9(e,i,Cc())}async _openRedirect(e,t,r,s){await this._originValidation(e);const i=await wh(e,t,r,Hl(),s);return I3(i),new Promise(()=>{})}_initialize(e){const t=e._key();if(this.eventManagers[t]){const{manager:s,promise:i}=this.eventManagers[t];return s?Promise.resolve(s):(vn(i,"If manager is not set, promise should be"),i)}const r=this.initAndGetManager(e);return this.eventManagers[t]={promise:r},r.catch(()=>{delete this.eventManagers[t]}),r}async initAndGetManager(e){const t=await c9(e),r=new G3(e);return t.register("authEvent",s=>(ne(s==null?void 0:s.authEvent,e,"invalid-auth-event"),{status:r.onEvent(s.authEvent)?"ACK":"ERROR"}),gapi.iframes.CROSS_ORIGIN_IFRAMES_FILTER),this.eventManagers[e._key()]={manager:r},this.iframes[e._key()]=t,r}_isIframeWebStorageSupported(e,t){this.iframes[e._key()].send(El,{type:El},s=>{var a;const i=(a=s==null?void 0:s[0])==null?void 0:a[El];i!==void 0&&t(!!i),qt(e,"internal-error")},gapi.iframes.CROSS_ORIGIN_IFRAMES_FILTER)}_originValidation(e){const t=e._key();return this.originValidationPromises[t]||(this.originValidationPromises[t]=J3(e)),this.originValidationPromises[t]}get _shouldInitProactively(){return cp()||rp()||Ic()}}const w9=E9;var bh="@firebase/auth",Ih="1.13.3";/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
`}),h}function il(o,h,f){e:{for(y in f){var y=!1;break e}y=!0}y||(f=f1(f),typeof o=="string"?f!=null&&xs(f):_e(o,h,f))}function Re(o){Ge.call(this),this.headers=new Map,this.L=o||null,this.h=!1,this.g=null,this.D="",this.o=0,this.l="",this.j=this.B=this.v=this.A=!1,this.m=null,this.F="",this.H=!1}p(Re,Ge);var y6=/^https?$/i,v6=["POST","PUT"];n=Re.prototype,n.Fa=function(o){this.H=o},n.ea=function(o,h,f,y){if(this.g)throw Error("[goog.net.XhrIo] Object is active with another request="+this.D+"; newUri="+o);h=h?h.toUpperCase():"GET",this.D=o,this.l="",this.o=0,this.A=!1,this.h=!0,this.g=this.L?this.L.g():Ku.g(),this.g.onreadystatechange=v(u(this.Ca,this));try{this.B=!0,this.g.open(h,String(o),!0),this.B=!1}catch(D){p1(this,D);return}if(o=f||"",f=new Map(this.headers),y)if(Object.getPrototypeOf(y)===Object.prototype)for(var P in y)f.set(P,y[P]);else if(typeof y.keys=="function"&&typeof y.get=="function")for(const D of y.keys())f.set(D,y.get(D));else throw Error("Unknown input type for opt_headers: "+String(y));y=Array.from(f.keys()).find(D=>D.toLowerCase()=="content-type"),P=a.FormData&&o instanceof a.FormData,!(Array.prototype.indexOf.call(v6,h,void 0)>=0)||y||P||f.set("Content-Type","application/x-www-form-urlencoded;charset=utf-8");for(const[D,$]of f)this.g.setRequestHeader(D,$);this.F&&(this.g.responseType=this.F),"withCredentials"in this.g&&this.g.withCredentials!==this.H&&(this.g.withCredentials=this.H);try{this.m&&(clearTimeout(this.m),this.m=null),this.v=!0,this.g.send(o),this.v=!1}catch(D){p1(this,D)}};function p1(o,h){o.h=!1,o.g&&(o.j=!0,o.g.abort(),o.j=!1),o.l=h,o.o=5,m1(o),Xi(o)}function m1(o){o.A||(o.A=!0,tt(o,"complete"),tt(o,"error"))}n.abort=function(o){this.g&&this.h&&(this.h=!1,this.j=!0,this.g.abort(),this.j=!1,this.o=o||7,tt(this,"complete"),tt(this,"abort"),Xi(this))},n.N=function(){this.g&&(this.h&&(this.h=!1,this.j=!0,this.g.abort(),this.j=!1),Xi(this,!0)),Re.Z.N.call(this)},n.Ca=function(){this.u||(this.B||this.v||this.j?g1(this):this.Xa())},n.Xa=function(){g1(this)};function g1(o){if(o.h&&typeof i<"u"){if(o.v&&Sn(o)==4)setTimeout(o.Ca.bind(o),0);else if(tt(o,"readystatechange"),Sn(o)==4){o.h=!1;try{const D=o.ca();e:switch(D){case 200:case 201:case 202:case 204:case 206:case 304:case 1223:var h=!0;break e;default:h=!1}var f;if(!(f=h)){var y;if(y=D===0){let $=String(o.D).match(i1)[1]||null;!$&&a.self&&a.self.location&&($=a.self.location.protocol.slice(0,-1)),y=!y6.test($?$.toLowerCase():"")}f=y}if(f)tt(o,"complete"),tt(o,"success");else{o.o=6;try{var P=Sn(o)>2?o.g.statusText:""}catch{P=""}o.l=P+" ["+o.ca()+"]",m1(o)}}finally{Xi(o)}}}}function Xi(o,h){if(o.g){o.m&&(clearTimeout(o.m),o.m=null);const f=o.g;o.g=null,h||tt(o,"ready");try{f.onreadystatechange=null}catch{}}}n.isActive=function(){return!!this.g};function Sn(o){return o.g?o.g.readyState:0}n.ca=function(){try{return Sn(this)>2?this.g.status:-1}catch{return-1}},n.la=function(){try{return this.g?this.g.responseText:""}catch{return""}},n.La=function(o){if(this.g){var h=this.g.responseText;return o&&h.indexOf(o)==0&&(h=h.substring(o.length)),Xm(h)}};function y1(o){try{if(!o.g)return null;if("response"in o.g)return o.g.response;switch(o.F){case"":case"text":return o.g.responseText;case"arraybuffer":if("mozResponseArrayBuffer"in o.g)return o.g.mozResponseArrayBuffer}return null}catch{return null}}function _6(o){const h={};o=(o.g&&Sn(o)>=2&&o.g.getAllResponseHeaders()||"").split(`\r
`);for(let y=0;y<o.length;y++){if(E(o[y]))continue;var f=r6(o[y]);const P=f[0];if(f=f[1],typeof f!="string")continue;f=f.trim();const D=h[P]||[];h[P]=D,D.push(f)}zm(h,function(y){return y.join(", ")})}n.ya=function(){return this.o},n.Ha=function(){return typeof this.l=="string"?this.l:String(this.l)};function Us(o,h,f){return f&&f.internalChannelParams&&f.internalChannelParams[o]||h}function v1(o){this.za=0,this.i=[],this.j=new Ss,this.ba=this.na=this.J=this.W=this.g=this.wa=this.G=this.H=this.u=this.U=this.o=null,this.Ya=this.V=0,this.Sa=Us("failFast",!1,o),this.F=this.C=this.v=this.m=this.l=null,this.X=!0,this.xa=this.K=-1,this.Y=this.A=this.D=0,this.Qa=Us("baseRetryDelayMs",5e3,o),this.Za=Us("retryDelaySeedMs",1e4,o),this.Ta=Us("forwardChannelMaxRetries",2,o),this.va=Us("forwardChannelRequestTimeoutMs",2e4,o),this.ma=o&&o.xmlHttpFactory||void 0,this.Ua=o&&o.Rb||void 0,this.Aa=o&&o.useFetchStreams||!1,this.O=void 0,this.L=o&&o.supportsCrossDomainXhr||!1,this.M="",this.h=new e1(o&&o.concurrentRequestLimit),this.Ba=new g6,this.S=o&&o.fastHandshake||!1,this.R=o&&o.encodeInitMessageHeaders||!1,this.S&&this.R&&(this.R=!1),this.Ra=o&&o.Pb||!1,o&&o.ua&&this.j.ua(),o&&o.forceLongPolling&&(this.X=!1),this.aa=!this.S&&this.X&&o&&o.detectBufferingProxy||!1,this.ia=void 0,o&&o.longPollingTimeout&&o.longPollingTimeout>0&&(this.ia=o.longPollingTimeout),this.ta=void 0,this.T=0,this.P=!1,this.ja=this.B=null}n=v1.prototype,n.ka=8,n.I=1,n.connect=function(o,h,f,y){nt(0),this.W=o,this.H=h||{},f&&y!==void 0&&(this.H.OSID=f,this.H.OAID=y),this.F=this.X,this.J=C1(this,null,this.W),Zi(this)};function al(o){if(_1(o),o.I==3){var h=o.V++,f=$t(o.J);if(_e(f,"SID",o.M),_e(f,"RID",h),_e(f,"TYPE","terminate"),Bs(o,f),h=new Tn(o,o.j,h),h.M=2,h.A=Qi($t(f)),f=!1,a.navigator&&a.navigator.sendBeacon)try{f=a.navigator.sendBeacon(h.A.toString(),"")}catch{}!f&&a.Image&&(new Image().src=h.A,f=!0),f||(h.g=S1(h.j,null),h.g.ea(h.A)),h.F=Date.now(),Ki(h)}R1(o)}function Ji(o){o.g&&(ll(o),o.g.cancel(),o.g=null)}function _1(o){Ji(o),o.v&&(a.clearTimeout(o.v),o.v=null),ea(o),o.h.cancel(),o.m&&(typeof o.m=="number"&&a.clearTimeout(o.m),o.m=null)}function Zi(o){if(!t1(o.h)&&!o.m){o.m=!0;var h=o.Ea;q||g(),K||(q(),K=!0),T.add(h,o),o.D=0}}function A6(o,h){return n1(o.h)>=o.h.j-(o.m?1:0)?!1:o.m?(o.i=h.G.concat(o.i),!0):o.I==1||o.I==2||o.D>=(o.Sa?0:o.Ta)?!1:(o.m=Cs(u(o.Ea,o,h),T1(o,o.D)),o.D++,!0)}n.Ea=function(o){if(this.m)if(this.m=null,this.I==1){if(!o){this.V=Math.floor(Math.random()*1e5),o=this.V++;const P=new Tn(this,this.j,o);let D=this.o;if(this.U&&(D?(D=ku(D),Du(D,this.U)):D=this.U),this.u!==null||this.R||(P.J=D,D=null),this.S)e:{for(var h=0,f=0;f<this.i.length;f++){t:{var y=this.i[f];if("__data__"in y.map&&(y=y.map.__data__,typeof y=="string")){y=y.length;break t}y=void 0}if(y===void 0)break;if(h+=y,h>4096){h=f;break e}if(h===4096||f===this.i.length-1){h=f+1;break e}}h=1e3}else h=1e3;h=E1(this,P,h),f=$t(this.J),_e(f,"RID",o),_e(f,"CVER",22),this.G&&_e(f,"X-HTTP-Session-Id",this.G),Bs(this,f),D&&(this.R?h="headers="+xs(f1(D))+"&"+h:this.u&&il(f,this.u,D)),nl(this.h,P),this.Ra&&_e(f,"TYPE","init"),this.S?(_e(f,"$req",h),_e(f,"SID","null"),P.U=!0,Jo(P,f,null)):Jo(P,f,h),this.I=2}}else this.I==3&&(o?A1(this,o):this.i.length==0||t1(this.h)||A1(this))};function A1(o,h){var f;h?f=h.l:f=o.V++;const y=$t(o.J);_e(y,"SID",o.M),_e(y,"RID",f),_e(y,"AID",o.K),Bs(o,y),o.u&&o.o&&il(y,o.u,o.o),f=new Tn(o,o.j,f,o.D+1),o.u===null&&(f.J=o.o),h&&(o.i=h.G.concat(o.i)),h=E1(o,f,1e3),f.H=Math.round(o.va*.5)+Math.round(o.va*.5*Math.random()),nl(o.h,f),Jo(f,y,h)}function Bs(o,h){o.H&&Fr(o.H,function(f,y){_e(h,y,f)}),o.l&&Fr({},function(f,y){_e(h,y,f)})}function E1(o,h,f){f=Math.min(o.i.length,f);const y=o.l?u(o.l.Ka,o.l,o):null;e:{var P=o.i;let le=-1;for(;;){const Ue=["count="+f];le==-1?f>0?(le=P[0].g,Ue.push("ofs="+le)):le=0:Ue.push("ofs="+le);let ve=!0;for(let qe=0;qe<f;qe++){var D=P[qe].g;const zt=P[qe].map;if(D-=le,D<0)le=Math.max(0,P[qe].g-100),ve=!1;else try{D="req"+D+"_"||"";try{var $=zt instanceof Map?zt:Object.entries(zt);for(const[dr,xn]of $){let Pn=xn;l(xn)&&(Pn=Go(xn)),Ue.push(D+dr+"="+encodeURIComponent(Pn))}}catch(dr){throw Ue.push(D+"type="+encodeURIComponent("_badmap")),dr}}catch{y&&y(zt)}}if(ve){$=Ue.join("&");break e}}$=void 0}return o=o.i.splice(0,f),h.G=o,$}function w1(o){if(!o.g&&!o.v){o.Y=1;var h=o.Da;q||g(),K||(q(),K=!0),T.add(h,o),o.A=0}}function ol(o){return o.g||o.v||o.A>=3?!1:(o.Y++,o.v=Cs(u(o.Da,o),T1(o,o.A)),o.A++,!0)}n.Da=function(){if(this.v=null,b1(this),this.aa&&!(this.P||this.g==null||this.T<=0)){var o=4*this.T;this.j.info("BP detection timer enabled: "+o),this.B=Cs(u(this.Wa,this),o)}},n.Wa=function(){this.B&&(this.B=null,this.j.info("BP detection timeout reached."),this.j.info("Buffering proxy detected and switch to long-polling!"),this.F=!1,this.P=!0,nt(10),Ji(this),b1(this))};function ll(o){o.B!=null&&(a.clearTimeout(o.B),o.B=null)}function b1(o){o.g=new Tn(o,o.j,"rpc",o.Y),o.u===null&&(o.g.J=o.o),o.g.P=0;var h=$t(o.na);_e(h,"RID","rpc"),_e(h,"SID",o.M),_e(h,"AID",o.K),_e(h,"CI",o.F?"0":"1"),!o.F&&o.ia&&_e(h,"TO",o.ia),_e(h,"TYPE","xmlhttp"),Bs(o,h),o.u&&o.o&&il(h,o.u,o.o),o.O&&(o.g.H=o.O);var f=o.g;o=o.ba,f.M=1,f.A=Qi($t(h)),f.u=null,f.R=!0,Xu(f,o)}n.Va=function(){this.C!=null&&(this.C=null,Ji(this),ol(this),nt(19))};function ea(o){o.C!=null&&(a.clearTimeout(o.C),o.C=null)}function I1(o,h){var f=null;if(o.g==h){ea(o),ll(o),o.g=null;var y=2}else if(tl(o.h,h))f=h.G,r1(o.h,h),y=1;else return;if(o.I!=0){if(h.o)if(y==1){f=h.u?h.u.length:0,h=Date.now()-h.F;var P=o.D;y=Wi(),tt(y,new Wu(y,f)),Zi(o)}else w1(o);else if(P=h.m,P==3||P==0&&h.X>0||!(y==1&&A6(o,h)||y==2&&ol(o)))switch(f&&f.length>0&&(h=o.h,h.i=h.i.concat(f)),P){case 1:hr(o,5);break;case 4:hr(o,10);break;case 3:hr(o,6);break;default:hr(o,2)}}}function T1(o,h){let f=o.Qa+Math.floor(Math.random()*o.Za);return o.isActive()||(f*=2),f*h}function hr(o,h){if(o.j.info("Error code "+h),h==2){var f=u(o.bb,o),y=o.Ua;const P=!y;y=new Rn(y||"//www.google.com/images/cleardot.gif"),a.location&&a.location.protocol=="http"||ks(y,"https"),Qi(y),P?p6(y.toString(),f):m6(y.toString(),f)}else nt(2);o.I=0,o.l&&o.l.pa(h),R1(o),_1(o)}n.bb=function(o){o?(this.j.info("Successfully pinged google.com"),nt(2)):(this.j.info("Failed to ping google.com"),nt(1))};function R1(o){if(o.I=0,o.ja=[],o.l){const h=s1(o.h);(h.length!=0||o.i.length!=0)&&(b(o.ja,h),b(o.ja,o.i),o.h.i.length=0,w(o.i),o.i.length=0),o.l.oa()}}function C1(o,h,f){var y=f instanceof Rn?$t(f):new Rn(f);if(y.g!="")h&&(y.g=h+"."+y.g),Ns(y,y.u);else{var P=a.location;y=P.protocol,h=h?h+"."+P.hostname:P.hostname,P=+P.port;const D=new Rn(null);y&&ks(D,y),h&&(D.g=h),P&&Ns(D,P),f&&(D.h=f),y=D}return f=o.G,h=o.wa,f&&h&&_e(y,f,h),_e(y,"VER",o.ka),Bs(o,y),y}function S1(o,h,f){if(h&&!o.L)throw Error("Can't create secondary domain capable XhrIo object.");return h=o.Aa&&!o.ma?new Re(new sl({ab:f})):new Re(o.ma),h.Fa(o.L),h}n.isActive=function(){return!!this.l&&this.l.isActive(this)};function x1(){}n=x1.prototype,n.ra=function(){},n.qa=function(){},n.pa=function(){},n.oa=function(){},n.isActive=function(){return!0},n.Ka=function(){};function ta(){}ta.prototype.g=function(o,h){return new _t(o,h)};function _t(o,h){Ge.call(this),this.g=new v1(h),this.l=o,this.h=h&&h.messageUrlParams||null,o=h&&h.messageHeaders||null,h&&h.clientProtocolHeaderRequired&&(o?o["X-Client-Protocol"]="webchannel":o={"X-Client-Protocol":"webchannel"}),this.g.o=o,o=h&&h.initMessageHeaders||null,h&&h.messageContentType&&(o?o["X-WebChannel-Content-Type"]=h.messageContentType:o={"X-WebChannel-Content-Type":h.messageContentType}),h&&h.sa&&(o?o["X-WebChannel-Client-Profile"]=h.sa:o={"X-WebChannel-Client-Profile":h.sa}),this.g.U=o,(o=h&&h.Qb)&&!E(o)&&(this.g.u=o),this.A=h&&h.supportsCrossDomainXhr||!1,this.v=h&&h.sendRawJson||!1,(h=h&&h.httpSessionIdParam)&&!E(h)&&(this.g.G=h,o=this.h,o!==null&&h in o&&(o=this.h,h in o&&delete o[h])),this.j=new jr(this)}p(_t,Ge),_t.prototype.m=function(){this.g.l=this.j,this.A&&(this.g.L=!0),this.g.connect(this.l,this.h||void 0)},_t.prototype.close=function(){al(this.g)},_t.prototype.o=function(o){var h=this.g;if(typeof o=="string"){var f={};f.__data__=o,o=f}else this.v&&(f={},f.__data__=Go(o),o=f);h.i.push(new a6(h.Ya++,o)),h.I==3&&Zi(h)},_t.prototype.N=function(){this.g.l=null,delete this.j,al(this.g),delete this.g,_t.Z.N.call(this)};function P1(o){Ko.call(this),o.__headers__&&(this.headers=o.__headers__,this.statusCode=o.__status__,delete o.__headers__,delete o.__status__);var h=o.__sm__;if(h){e:{for(const f in h){o=f;break e}o=void 0}(this.i=o)&&(o=this.i,h=h!==null&&o in h?h[o]:void 0),this.data=h}else this.data=o}p(P1,Ko);function k1(){Qo.call(this),this.status=1}p(k1,Qo);function jr(o){this.g=o}p(jr,x1),jr.prototype.ra=function(){tt(this.g,"a")},jr.prototype.qa=function(o){tt(this.g,new P1(o))},jr.prototype.pa=function(o){tt(this.g,new k1)},jr.prototype.oa=function(){tt(this.g,"b")},ta.prototype.createWebChannel=ta.prototype.g,_t.prototype.send=_t.prototype.o,_t.prototype.open=_t.prototype.m,_t.prototype.close=_t.prototype.close,Op=function(){return new ta},Dp=function(){return Wi()},Np=lr,$l={jb:0,mb:1,nb:2,Hb:3,Mb:4,Jb:5,Kb:6,Ib:7,Gb:8,Lb:9,PROXY:10,NOPROXY:11,Eb:12,Ab:13,Bb:14,zb:15,Cb:16,Db:17,fb:18,eb:19,gb:20},Gi.NO_ERROR=0,Gi.TIMEOUT=8,Gi.HTTP_ERROR=6,_a=Gi,Gu.COMPLETE="complete",kp=Gu,Hu.EventType=Ts,Ts.OPEN="a",Ts.CLOSE="b",Ts.ERROR="c",Ts.MESSAGE="d",Ge.prototype.listen=Ge.prototype.J,Ws=Hu,Re.prototype.listenOnce=Re.prototype.K,Re.prototype.getLastError=Re.prototype.Ha,Re.prototype.getLastErrorCode=Re.prototype.ya,Re.prototype.getStatus=Re.prototype.ca,Re.prototype.getResponseJson=Re.prototype.La,Re.prototype.getResponseText=Re.prototype.la,Re.prototype.send=Re.prototype.ea,Re.prototype.setWithCredentials=Re.prototype.Fa,Pp=Re}).apply(typeof ia<"u"?ia:typeof self<"u"?self:typeof window<"u"?window:{});/*!
 * re2js
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Cr=new lo("@firebase/firestore");function zr(){return Cr.logLevel}function W(n,...e){if(Cr.logLevel<=he.DEBUG){const t=e.map(xc);Cr.debug(`Firestore (${ps}): ${n}`,...t)}}function _n(n,...e){if(Cr.logLevel<=he.ERROR){const t=e.map(xc);Cr.error(`Firestore (${ps}): ${n}`,...t)}}function Ht(n,...e){if(Cr.logLevel<=he.WARN){const t=e.map(xc);Cr.warn(`Firestore (${ps}): ${n}`,...t)}}function xc(n){if(typeof n=="string")return n;try{return(function(t){return JSON.stringify(t)})(n)}catch{return n}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Z(n,e,t){let r="Unexpected state";typeof e=="string"?r=e:t=e,Vp(n,r,t)}function Vp(n,e,t){let r=`FIRESTORE (${ps}) INTERNAL ASSERTION FAILED: ${e} (ID: ${n.toString(16)})`;if(t!==void 0)try{r+=" CONTEXT: "+JSON.stringify(t)}catch{r+=" CONTEXT: "+t}throw _n(r),new Error(r)}function Q(n,e,t,r){let s="Unexpected state";typeof t=="string"?s=t:r=t,n||Vp(e,s,r)}function ie(n,e){return n}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const kc=-1;function _o(n){return n==null}function fi(n){return n===0&&1/n==-1/0}function o8(n){return typeof n=="number"&&Number.isInteger(n)&&!fi(n)&&n<=Number.MAX_SAFE_INTEGER&&n>=Number.MIN_SAFE_INTEGER}function l8(n){return typeof n=="string"}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Tt{constructor(e){this.value=e}static empty(){return new Tt({mapValue:{}})}field(e){if(e.isEmpty())return this.value;{let t=this.value;for(let r=0;r<e.length-1;++r)if(t=(t.mapValue.fields||{})[e.get(r)],!vr(t))return null;return t=(t.mapValue.fields||{})[e.lastSegment()],t||null}}set(e,t){this.getFieldsMap(e.popLast())[e.lastSegment()]=Zs(t)}setAll(e){let t=Be.emptyPath(),r={},s=[];e.forEach(((a,l)=>{if(!t.isImmediateParentOf(l)){const c=this.getFieldsMap(t);this.applyChanges(c,r,s),r={},s=[],t=l.popLast()}a?r[l.lastSegment()]=Zs(a):s.push(l.lastSegment())}));const i=this.getFieldsMap(t);this.applyChanges(i,r,s)}delete(e){const t=this.field(e.popLast());vr(t)&&t.mapValue.fields&&delete t.mapValue.fields[e.lastSegment()]}isEqual(e){return Ot(this.value,e.value)}getFieldsMap(e){let t=this.value;t.mapValue.fields||(t.mapValue={fields:{}});for(let r=0;r<e.length;++r){let s=t.mapValue.fields[e.get(r)];vr(s)&&s.mapValue.fields||(s={mapValue:{fields:{}}},t.mapValue.fields[e.get(r)]=s),t=s}return t.mapValue.fields}applyChanges(e,t,r){Or(t,((s,i)=>e[s]=i));for(const s of r)delete e[s]}clone(){return new Tt(Zs(this.value))}}function Kp(n){const e=[];return Or(n.fields,((t,r)=>{const s=new Be([t]);if(vr(r)){const i=Kp(r.mapValue).fields;if(i.length===0)e.push(s);else for(const a of i)e.push(s.child(a))}else e.push(s)})),new Ut(e)}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class wo{constructor(){this._=void 0}}function y8(n,e,t){return n instanceof yi?(function(s,i){const a={fields:{[jp]:{stringValue:Hp},[zp]:{timestampValue:{seconds:s.seconds,nanos:s.nanoseconds}}}};return i&&Ao(i)&&(i=Vi(i)),i&&(a.fields[$p]=i),{mapValue:a}})(t,e):n instanceof vi?Yp(n,e):n instanceof _i?Xp(n,e):n instanceof Ai?(function(s,i){const a=Qp(s,i),l=Ga(a)+Ga(s.Re);return Kt(a)&&Kt(s.Re)?Nc(l):Eo(s.serializer,l)})(n,e):n instanceof za?(function(s,i){return sd(s,i,Math.min)})(n,e):n instanceof Wa?(function(s,i){return sd(s,i,Math.max)})(n,e):void 0}function v8(n,e,t){return n instanceof vi?Yp(n,e):n instanceof _i?Xp(n,e):t}function Qp(n,e){return n instanceof Ai?er(e)?e:{integerValue:0}:null}class yi extends wo{}class vi extends wo{constructor(e){super(),this.elements=e}}function Yp(n,e){const t=Jp(e);for(const r of n.elements)t.some((s=>Ot(s,r)))||t.push(r);return{arrayValue:{values:t}}}class _i extends wo{constructor(e){super(),this.elements=e}}function Xp(n,e){let t=Jp(e);for(const r of n.elements)t=t.filter((s=>!Ot(s,r)));return{arrayValue:{values:t}}}class Oc extends wo{constructor(e,t){super(),this.serializer=e,this.Re=t}}class Ai extends Oc{}class za extends Oc{}class Wa extends Oc{}function sd(n,e,t){if(!er(e))return n.Re;const r=t(Ga(e),Ga(n.Re));return Kt(e)&&Kt(n.Re)?Nc(r):Eo(n.serializer,r)}function Ga(n){return Ie(n.integerValue||n.doubleValue)}function Jp(n){return us(n)&&n.arrayValue.values?n.arrayValue.values.slice():[]}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Pt{constructor(e){this._byteString=e}static fromBase64String(e){try{return new Pt(Ve.fromBase64String(e))}catch(t){throw new z(M.INVALID_ARGUMENT,"Failed to construct data from Base64 string: "+t)}}static fromUint8Array(e){return new Pt(Ve.fromUint8Array(e))}toBase64(){return this._byteString.toBase64()}toUint8Array(){return this._byteString.toUint8Array()}toString(){return"Bytes(base64: "+this.toBase64()+")"}isEqual(e){return this._byteString.isEqual(e._byteString)}toJSON(){return{type:Pt._jsonSchemaVersion,bytes:this.toBase64()}}static fromJSON(e){if(Li(e,Pt._jsonSchema))return Pt.fromBase64String(e.bytes)}}Pt._jsonSchemaVersion="firestore/bytes/1.0",Pt._jsonSchema={type:Oe("string",Pt._jsonSchemaVersion),bytes:Oe("string")};/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const yd="ConnectivityMonitor";class vd{constructor(){this.vt=()=>this.St(),this.Dt=()=>this.xt(),this.Ct=[],this.Ft()}bt(e){this.Ct.push(e)}shutdown(){window.removeEventListener("online",this.vt),window.removeEventListener("offline",this.Dt)}Ft(){window.addEventListener("online",this.vt),window.addEventListener("offline",this.Dt)}St(){W(yd,"Network connectivity changed: AVAILABLE");for(const e of this.Ct)e(0)}xt(){W(yd,"Network connectivity changed: UNAVAILABLE");for(const e of this.Ct)e(1)}static C(){return typeof window<"u"&&window.addEventListener!==void 0&&window.removeEventListener!==void 0}}/**
 * @license
 * Copyright 2023 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class vy{constructor(e){this.Kt=e.Kt,this.Wt=e.Wt}Qt(e){this.Gt=e}zt(e){this.jt=e}Ht(e){this.Jt=e}onMessage(e){this.Yt=e}close(){this.Wt()}send(e){this.Kt(e)}Zt(){this.Gt()}Xt(){this.jt()}en(e){this.Jt(e)}tn(e){this.Yt(e)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Ke="WebChannelConnection",qs=(n,e,t)=>{n.listen(e,(r=>{try{t(r)}catch(s){setTimeout((()=>{throw s}),0)}}))};class rs extends yy{constructor(e){super(e),this.nn=[],this.forceLongPolling=e.forceLongPolling,this.autoDetectLongPolling=e.autoDetectLongPolling,this.useFetchStreams=e.useFetchStreams,this.longPollingOptions=e.longPollingOptions}static rn(){if(!rs.sn){const e=Dp();qs(e,Np.STAT_EVENT,(t=>{t.stat===$l.PROXY?W(Ke,"STAT_EVENT: detected buffering proxy"):t.stat===$l.NOPROXY&&W(Ke,"STAT_EVENT: detected no buffering proxy")})),rs.sn=!0}}qt(e,t,r,s,i){const a=ec();return new Promise(((l,c)=>{const u=new Pp;u.setWithCredentials(!0),u.listenOnce(kp.COMPLETE,(()=>{try{switch(u.getLastErrorCode()){case _a.NO_ERROR:const p=u.getResponseJson();W(Ke,`XHR for RPC '${e}' ${a} received:`,JSON.stringify(p)),l(p);break;case _a.TIMEOUT:W(Ke,`RPC '${e}' ${a} timed out`),c(new z(M.DEADLINE_EXCEEDED,"Request time out"));break;case _a.HTTP_ERROR:const v=u.getStatus();if(W(Ke,`RPC '${e}' ${a} failed with status:`,v,"response text:",u.getResponseText()),v>0){let w=u.getResponseJson();Array.isArray(w)&&(w=w[0]);const b=w==null?void 0:w.error;if(b&&b.status&&b.message){const x=(function(B){const V=B.toLowerCase().replace(/_/g,"-");return Object.values(M).indexOf(V)>=0?V:M.UNKNOWN})(b.status);c(new z(x,b.message))}else c(new z(M.UNKNOWN,"Server responded with status "+u.getStatus()))}else c(new z(M.UNAVAILABLE,"Connection failed."));break;default:Z(9055,{_n:e,streamId:a,an:u.getLastErrorCode(),un:u.getLastError()})}}finally{W(Ke,`RPC '${e}' ${a} completed.`)}}));const d=JSON.stringify(s);W(Ke,`RPC '${e}' ${a} sending request:`,s),u.send(t,"POST",d,r,15)}))}cn(e,t,r){const s=ec(),i=[this.Mt,"/","google.firestore.v1.Firestore","/",e,"/channel"],a=this.createWebChannelTransport(),l={httpSessionIdParam:"gsessionid",initMessageHeaders:{},messageUrlParams:{database:`projects/${this.databaseId.projectId}/databases/${this.databaseId.database}`},sendRawJson:!0,supportsCrossDomainXhr:!0,internalChannelParams:{forwardChannelRequestTimeoutMs:6e5},forceLongPolling:this.forceLongPolling,detectBufferingProxy:this.autoDetectLongPolling},c=this.longPollingOptions.timeoutSeconds;c!==void 0&&(l.longPollingTimeout=Math.round(1e3*c)),this.useFetchStreams&&(l.useFetchStreams=!0),this.kt(l.initMessageHeaders,t,r),l.encodeInitMessageHeaders=!0;const u=i.join("");W(Ke,`Creating RPC '${e}' stream ${s}: ${u}`,l);const d=a.createWebChannel(u,l);this.En(d);let p=!1,v=!1;const w=new vy({Kt:b=>{v?W(Ke,`Not sending because RPC '${e}' stream ${s} is closed:`,b):(p||(W(Ke,`Opening RPC '${e}' stream ${s} transport.`),d.open(),p=!0),W(Ke,`RPC '${e}' stream ${s} sending:`,b),d.send(b))},Wt:()=>d.close()});return qs(d,Ws.EventType.OPEN,(()=>{v||(W(Ke,`RPC '${e}' stream ${s} transport opened.`),w.Zt())})),qs(d,Ws.EventType.CLOSE,(()=>{v||(v=!0,W(Ke,`RPC '${e}' stream ${s} transport closed`),w.en(),this.hn(d))})),qs(d,Ws.EventType.ERROR,(b=>{v||(v=!0,Ht(Ke,`RPC '${e}' stream ${s} transport errored. Name:`,b.name,"Message:",b.message),w.en(new z(M.UNAVAILABLE,"The operation could not be completed")))})),qs(d,Ws.EventType.MESSAGE,(b=>{var x;if(!v){const k=b.data[0];Q(!!k,16349);const B=k,V=(B==null?void 0:B.error)||((x=B[0])==null?void 0:x.error);if(V){W(Ke,`RPC '${e}' stream ${s} received error:`,V);const F=V.status;let j=(function(T){const g=Ne[T];if(g!==void 0)return d2(g)})(F),q=V.message;F==="NOT_FOUND"&&q.includes("database")&&q.includes("does not exist")&&q.includes(this.databaseId.database)&&Ht(`Database '${this.databaseId.database}' not found. Please check your project configuration.`),j===void 0&&(j=M.INTERNAL,q="Unknown error status: "+F+" with message "+V.message),v=!0,w.en(new z(j,q)),d.close()}else W(Ke,`RPC '${e}' stream ${s} received:`,k),w.tn(k)}})),rs.rn(),setTimeout((()=>{w.Xt()}),0),w}terminate(){this.nn.forEach((e=>e.close())),this.nn=[]}En(e){this.nn.push(e)}hn(e){this.nn=this.nn.filter((t=>t===e))}kt(e,t,r){super.kt(e,t,r),this.databaseInfo.apiKey&&(e["x-goog-api-key"]=this.databaseInfo.apiKey)}createWebChannelTransport(){return Op()}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class mv{constructor(e){this.zr=e}}function gv(n){const e=oy({parent:n.parent,structuredQuery:n.structuredQuery});return n.limitType==="LAST"?Qa(e,e.limit,"L"):e}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class yv{constructor(){this.Hi=new vv}addToCollectionParentIndex(e,t){return this.Hi.add(t),U.resolve()}getCollectionParents(e,t){return U.resolve(this.Hi.getEntries(t))}addFieldIndex(e,t){return U.resolve()}deleteFieldIndex(e,t){return U.resolve()}deleteAllFieldIndexes(e){return U.resolve()}createTargetIndexes(e,t){return U.resolve()}getDocumentsMatchingTarget(e,t){return U.resolve(null)}getIndexType(e,t){return U.resolve(0)}getFieldIndexes(e,t){return U.resolve([])}getNextCollectionGroupToUpdate(e){return U.resolve(null)}getMinOffset(e,t){return U.resolve(Xn.min())}getMinOffsetFromCollectionGroup(e,t){return U.resolve(Xn.min())}updateCollectionGroup(e,t,r){return U.resolve()}updateIndexEntries(e,t){return U.resolve()}}class vv{constructor(){this.index={}}add(e){const t=e.lastSegment(),r=e.popLast(),s=this.index[t]||new Le(me.comparator),i=!s.has(r);return this.index[t]=s.add(r),i}has(e){const t=e.lastSegment(),r=e.popLast(),s=this.index[t];return s&&s.has(r)}getEntries(e){return(this.index[e]||new Le(me.comparator)).toArray()}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
function em(n,e){var r;let t=e;for(const s of n.stages)t=Av({serializer:n.serializer,serverTimestampBehavior:(r=n.listenOptions)==null?void 0:r.serverTimestampBehavior},s,t);return t}function Oo(n,e){return em(n,[e]).length>0}function _v(n,e){return Fe(n)?Oo(n,e):To(n,e)}function Av(n,e,t){if(e instanceof So)return(function(s,i,a){return a.filter((l=>l.isFoundDocument()&&`/${l.key.getCollectionPath().canonicalString()}`===i.Vr))})(0,e,t);if(e instanceof Po)return(function(s,i,a){return a.filter((l=>{const c=ii(ee(i.condition).evaluate(s,l));return c!==void 0&&Ot(c,gt)}))})(n,e,t);if(e instanceof xo)return(function(s,i,a){return a.filter((l=>l.isFoundDocument()&&l.key.getCollectionPath().lastSegment()===i.collectionId))})(0,e,t);if(e instanceof zc)return(function(s,i,a){return a.filter((l=>l.isFoundDocument()))})(0,0,t);if(e instanceof Wc)return(function(s,i,a){return a.filter((l=>l.isFoundDocument()&&i.mr.has(l.key.path.toStringWithLeadingSlash())))})(0,e,t);if(e instanceof Sr)return(function(s,i,a){return a.slice(0,i.limit)})(0,e,t);if(e instanceof hn)return(function(s,i,a){const l=i.orderings.map((c=>({ks:ee(c.expr),direction:c.direction})));return[...a].sort(((c,u)=>{for(const{ks:d,direction:p}of l){const v=ii(d.evaluate(s,c)),w=ii(d.evaluate(s,u)),b=yt(v??ls,w??ls);if(b!==0)return p==="ascending"?b:-b}return 0}))})(n,e,t);throw new Error(`Unknown stage: ${e._name}`)}function nc(n){const e=(function(r){for(let s=r.stages.length-1;s>=0;s--){const i=r.stages[s];if(i instanceof hn)return i.orderings}throw new Error("Pipeline must contain at least one Sort stage")})(n);return(t,r)=>{for(const s of e){const i=ii(ee(s.expr).evaluate({serializer:n.serializer},t)),a=ii(ee(s.expr).evaluate({serializer:n.serializer},r)),l=yt(i||ls,a||ls);if(l!==0)return s.direction==="ascending"?l:-l}return 0}}function Rl(n){for(let e=n.stages.length-1;e>=0;e--){const t=n.stages[e];if(t instanceof Sr)return{limit:t.limit}}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Iv{constructor(e){this.serializer=e,this.Hs=new Map,this.Js=new Map}getBundleMetadata(e,t){return U.resolve(this.Hs.get(t))}saveBundleMetadata(e,t){return this.Hs.set(t.id,(function(s){return{id:s.id,version:s.version,createTime:en(s.createTime)}})(t)),U.resolve()}getNamedQuery(e,t){return U.resolve(this.Js.get(t))}saveNamedQuery(e,t){return this.Js.set(t.name,(function(s){return{name:s.name,query:gv(s.bundledQuery),readTime:en(s.readTime)}})(t)),U.resolve()}}/**
 * @license
 * Copyright 2022 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Tv{constructor(){this.overlays=new we(X.comparator),this.Ys=new Map}getOverlay(e,t){return U.resolve(this.overlays.get(t))}getOverlays(e,t){const r=Bn();return U.forEach(t,(s=>this.getOverlay(e,s).next((i=>{i!==null&&r.set(s,i)})))).next((()=>r))}getAllOverlays(e,t){const r=Bn();return this.overlays.forEach(((s,i)=>{i.largestBatchId>t&&r.set(s,i)})),U.resolve(r)}saveOverlays(e,t,r){return r.forEach(((s,i)=>{this.Hr(e,t,i)})),U.resolve()}removeOverlaysForBatchId(e,t,r){const s=this.Ys.get(r);return s!==void 0&&(s.forEach((i=>this.overlays=this.overlays.remove(i))),this.Ys.delete(r)),U.resolve()}getOverlaysForCollection(e,t,r){const s=Bn(),i=t.length+1,a=new X(t.child("")),l=this.overlays.getIteratorFrom(a);for(;l.hasNext();){const c=l.getNext().value,u=c.getKey();if(!t.isPrefixOf(u.path))break;u.path.length===i&&c.largestBatchId>r&&s.set(c.getKey(),c)}return U.resolve(s)}getOverlaysForCollectionGroup(e,t,r,s){let i=new we(((u,d)=>u-d));const a=this.overlays.getIterator();for(;a.hasNext();){const u=a.getNext().value;if(u.getKey().getCollectionGroup()===t&&u.largestBatchId>r){let d=i.get(u.largestBatchId);d===null&&(d=Bn(),i=i.insert(u.largestBatchId,d)),d.set(u.getKey(),u)}}const l=Bn(),c=i.getIterator();for(;c.hasNext()&&(c.getNext().value.forEach(((u,d)=>l.set(u,d))),!(l.size()>=s)););return U.resolve(l)}Hr(e,t,r){const s=this.overlays.get(r.key);if(s!==null){const a=this.Ys.get(s.largestBatchId).delete(r.key);this.Ys.set(s.largestBatchId,a)}this.overlays=this.overlays.insert(r.key,new pv(t,r));let i=this.Ys.get(t);i===void 0&&(i=ce(),this.Ys.set(t,i)),this.Ys.set(t,i.add(r.key))}}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Rv{constructor(){this.sessionToken=Ve.EMPTY_BYTE_STRING}getSessionToken(e){return U.resolve(this.sessionToken)}setSessionToken(e,t){return this.sessionToken=t,U.resolve()}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Nv(n,e){return X.comparator(n.key,e.key)}/**
 * @license
 * Copyright 2023 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Ov{constructor(){this.So=!1,this.Do=!1,this.xo=100,this.Co=(function(){return Cg()?8:a8(Ze())>0?6:4})()}initialize(e,t){this.Fo=e,this.indexManager=t,this.So=!0}getDocumentsMatchingQuery(e,t,r,s){const i={result:null};return this.Oo(e,t).next((a=>{i.result=a})).next((()=>{if(!i.result)return this.Mo(e,t,s,r).next((a=>{i.result=a}))})).next((()=>{if(i.result)return;const a=new Dv;return this.No(e,t,a).next((l=>{if(i.result=l,this.Do)return this.Lo(e,t,a,l.size)}))})).next((()=>i.result))}Lo(e,t,r,s){return Fe(t)?U.resolve():r.documentReadCount<this.xo?(zr()<=he.DEBUG&&W("QueryEngine","SDK will not create cache indexes for query:",ni(t),"since it only creates cache indexes for collection contains","more than or equal to",this.xo,"documents"),U.resolve()):(zr()<=he.DEBUG&&W("QueryEngine","Query:",ni(t),"scans",r.documentReadCount,"local documents and returns",s,"documents as results."),r.documentReadCount>this.Co*s?(zr()<=he.DEBUG&&W("QueryEngine","The SDK decides to create cache indexes for query:",ni(t),"as using cache indexes may help improve performance."),this.indexManager.createTargetIndexes(e,Zt(t))):U.resolve())}Oo(e,t){if(Fe(t))return U.resolve(null);let r=t;if(hd(r))return U.resolve(null);let s=Zt(r);return this.indexManager.getIndexType(e,s).next((i=>i===0?null:(r.limit!==null&&i===1&&(r=Qa(r,null,"F"),s=Zt(r)),this.indexManager.getDocumentsMatchingTarget(e,s).next((a=>{const l=ce(...a);return this.Fo.getDocuments(e,l).next((c=>this.indexManager.getMinOffset(e,s).next((u=>{const d=this.Bo(r,c);return this.Uo(r,d,l,u.readTime)?this.Oo(e,Qa(r,null,"F")):this.ko(e,d,r,u)}))))})))))}Mo(e,t,r,s){return(Fe(t)?(function(a){for(const l of a.stages){if(l instanceof Sr||l instanceof Rd)return!1;if(l instanceof Po){if(l.condition instanceof B2&&l.condition._expr.name==="exists"&&l.condition._expr.params[0]instanceof _s&&l.condition._expr.params[0].fieldName===Gt)continue;return!1}}return!0})(t):hd(t))||s.isEqual(se.min())?U.resolve(null):this.Fo.getDocuments(e,r).next((i=>{const a=this.Bo(t,i);return this.Uo(t,a,r,s)?U.resolve(null):(zr()<=he.DEBUG&&W("QueryEngine","Re-using previous result from %s to execute query: %s",s.toString(),Cd(t)),this.ko(e,a,t,t8(s,di)).next((l=>l)))}))}Bo(e,t){let r,s;return Fe(e)?(r=new Le(Nv),s=i=>Oo(e,i)):(r=new Le(Lc(e)),s=i=>To(e,i)),t.forEach(((i,a)=>{s(a)&&(r=r.add(a))})),r}Uo(e,t,r,s){if(Fe(e))return(function(l){return l.stages.some((c=>c instanceof Sr||c instanceof Rd))})(e);if(e.limit===null)return!1;if(r.size!==t.size)return!0;const i=e.limitType==="F"?t.last():t.first();return!!i&&(i.hasPendingWrites||i.version.compareTo(s)>0)}No(e,t,r){return zr()<=he.DEBUG&&W("QueryEngine","Using full collection scan to execute query:",Cd(t)),this.Fo.getDocumentsMatchingQuery(e,t,Xn.min(),r)}ko(e,t,r,s){return this.Fo.getDocumentsMatchingQuery(e,r,s).next((i=>(t.forEach((a=>{i=i.insert(a.key,a)})),i)))}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const iu="LocalStore",Lv=3e8;class Vv{constructor(e,t,r,s){this.persistence=e,this.qo=t,this.serializer=s,this.$o=new we(ue),this.Ko=new Vr((i=>J2(i)),Z2),this.Wo=new Map,this.Qo=e.getRemoteDocumentCache(),this.g_=e.getTargetCache(),this.w_=e.getBundleCache(),this.Go(r)}Go(e){this.documentOverlayCache=this.persistence.getDocumentOverlayCache(e),this.indexManager=this.persistence.getIndexManager(e),this.mutationQueue=this.persistence.getMutationQueue(e,this.indexManager),this.localDocuments=new bv(this.Qo,this.mutationQueue,this.documentOverlayCache,this.indexManager),this.Qo.setIndexManager(this.indexManager),this.qo.initialize(this.localDocuments,this.indexManager)}collectGarbage(e){return this.persistence.runTransaction("Collect garbage","readwrite-primary",(t=>e.collect(t,this.$o)))}}function Mv(n,e,t,r){return new Vv(n,e,t,r)}async function nm(n,e){const t=ie(n);return await t.persistence.runTransaction("Handle user change","readonly",(r=>{let s;return t.mutationQueue.getAllMutationBatches(r).next((i=>(s=i,t.Go(e),t.mutationQueue.getAllMutationBatches(r)))).next((i=>{const a=[],l=[];let c=ce();for(const u of s){a.push(u.batchId);for(const d of u.mutations)c=c.add(d.key)}for(const u of i){l.push(u.batchId);for(const d of u.mutations)c=c.add(d.key)}return t.localDocuments.getDocuments(r,c).next((u=>({zo:u,removedBatchIds:a,addedBatchIds:l})))}))}))}function Uv(n,e){const t=ie(n);return t.persistence.runTransaction("Acknowledge batch","readwrite-primary",(r=>{const s=e.batch.keys(),i=t.Qo.newChangeBuffer({trackRemovals:!0});return(function(l,c,u,d){const p=u.batch,v=p.keys();let w=U.resolve();return v.forEach((b=>{w=w.next((()=>d.getEntry(c,b))).next((x=>{const k=u.docVersions.get(b);Q(k!==null,48541),x.version.compareTo(k)<0&&(p.applyToRemoteDocument(x,u),x.isValidDocument()&&(x.setReadTime(u.commitVersion),d.addEntry(x)))}))})),w.next((()=>l.mutationQueue.removeMutationBatch(c,p)))})(t,r,e,i).next((()=>i.apply(r))).next((()=>t.mutationQueue.performConsistencyCheck(r))).next((()=>t.documentOverlayCache.removeOverlaysForBatchId(r,s,e.batch.batchId))).next((()=>t.localDocuments.recalculateAndSaveOverlaysForDocumentKeys(r,(function(l){let c=ce();for(let u=0;u<l.mutationResults.length;++u)l.mutationResults[u].transformResults.length>0&&(c=c.add(l.batch.mutations[u].key));return c})(e)))).next((()=>t.localDocuments.getDocuments(r,s)))}))}function rm(n){const e=ie(n);return e.persistence.runTransaction("Get last remote snapshot version","readonly",(t=>e.g_.getLastRemoteSnapshotVersion(t)))}function Bv(n,e){const t=ie(n),r=e.snapshotVersion;let s=t.$o;return t.persistence.runTransaction("Apply remote event","readwrite-primary",(i=>{const a=t.Qo.newChangeBuffer({trackRemovals:!0});s=t.$o;const l=[];e.targetChanges.forEach(((d,p)=>{const v=s.get(p);if(!v)return;l.push(t.g_.removeMatchingKeys(i,d.removedDocuments,p).next((()=>t.g_.addMatchingKeys(i,d.addedDocuments,p))));let w=v.withSequenceNumber(i.currentSequenceNumber);e.targetMismatches.get(p)!==null?w=w.withResumeToken(Ve.EMPTY_BYTE_STRING,se.min()).withLastLimboFreeSnapshotVersion(se.min()):d.resumeToken.approximateByteSize()>0&&(w=w.withResumeToken(d.resumeToken,r)),s=s.insert(p,w),(function(x,k,B){return x.resumeToken.approximateByteSize()===0||k.snapshotVersion.toMicroseconds()-x.snapshotVersion.toMicroseconds()>=Lv?!0:B.addedDocuments.size+B.modifiedDocuments.size+B.removedDocuments.size>0})(v,w,d)&&l.push(t.g_.updateTargetData(i,w))}));let c=ft(),u=ce();if(e.documentUpdates.forEach((d=>{e.resolvedLimboDocuments.has(d)&&l.push(t.persistence.referenceDelegate.updateLimboDocument(i,d))})),l.push(Fv(i,a,e.documentUpdates).next((d=>{c=d.jo,u=d.Ho}))),!r.isEqual(se.min())){const d=t.g_.getLastRemoteSnapshotVersion(i).next((p=>t.g_.setTargetsMetadata(i,i.currentSequenceNumber,r)));l.push(d)}return U.waitFor(l).next((()=>a.apply(i))).next((()=>t.localDocuments.getLocalViewOfDocuments(i,c,u))).next((()=>c))})).then((i=>(t.$o=s,i)))}function Fv(n,e,t){let r=ce(),s=ce();return t.forEach((i=>r=r.add(i))),e.getEntries(n,r).next((i=>{let a=ft();return t.forEach(((l,c)=>{const u=i.get(l);c.isFoundDocument()!==u.isFoundDocument()&&(s=s.add(l)),c.isNoDocument()&&c.version.isEqual(se.min())?(e.removeEntry(l,c.readTime),a=a.insert(l,c)):!u.isValidDocument()||c.version.compareTo(u.version)>0||c.version.compareTo(u.version)===0&&u.hasPendingWrites?(e.addEntry(c),a=a.insert(l,c)):W(iu,"Ignoring outdated watch update for ",l,". Current version:",u.version," Watch version:",c.version)})),{jo:a,Ho:s}}))}function qv(n,e){const t=ie(n);return t.persistence.runTransaction("Get next mutation batch","readonly",(r=>(e===void 0&&(e=kc),t.mutationQueue.getNextMutationBatchAfterBatchId(r,e))))}function Hv(n,e){const t=ie(n);return t.persistence.runTransaction("Allocate target","readwrite",(r=>{let s;return t.g_.getTargetData(r,e).next((i=>i?(s=i,U.resolve(s)):t.g_.allocateTargetId(r).next((a=>(s=new dn(e,a,"TargetPurposeListen",r.currentSequenceNumber),t.g_.addTargetData(r,s).next((()=>s)))))))})).then((r=>{const s=t.$o.get(r.targetId);return(s===null||r.snapshotVersion.compareTo(s.snapshotVersion)>0)&&(t.$o=t.$o.insert(r.targetId,r),t.Ko.set(e,r.targetId)),r}))}async function rc(n,e,t){const r=ie(n),s=r.$o.get(e),i=t?"readwrite":"readwrite-primary";try{t||await r.persistence.runTransaction("Release target",i,(a=>r.persistence.referenceDelegate.removeTarget(a,s)))}catch(a){if(!gs(a))throw a;W(iu,`Failed to update sequence numbers for target ${e}: ${a}`)}r.$o=r.$o.remove(e),r.Ko.delete(s.target)}function Sd(n,e,t){const r=ie(n);let s=se.min(),i=ce();return r.persistence.runTransaction("Execute query","readwrite",(a=>(function(c,u,d){const p=ie(c),v=p.Ko.get(d);return v!==void 0?U.resolve(p.$o.get(v)):p.g_.getTargetData(u,d)})(r,a,Fe(e)?e:Zt(e)).next((l=>{if(l)return s=l.lastLimboFreeSnapshotVersion,r.g_.getMatchingKeysForTargetId(a,l.targetId).next((c=>{i=c}))})).next((()=>r.qo.getDocumentsMatchingQuery(a,e,t?s:se.min(),t?i:ce()))).next((l=>(jv(r,l),{documents:l,Jo:i})))))}function jv(n,e){e.forEach(((t,r)=>{const s=r.key.getCollectionGroup(),i=n.Wo.get(s)||se.min();r.readTime.compareTo(i)>0&&n.Wo.set(s,r.readTime)}))}class xd{constructor(){this.activeTargetIds=G8()}na(e){this.activeTargetIds=this.activeTargetIds.add(e)}ra(e){this.activeTargetIds=this.activeTargetIds.delete(e)}ta(){const e={activeTargetIds:this.activeTargetIds.toArray(),updateTimeMs:Date.now()};return JSON.stringify(e)}}class $v{constructor(){this.Ua=new xd,this.ka={},this.onlineStateHandler=null,this.sequenceNumberHandler=null}addPendingMutation(e){}updateMutationState(e,t,r){}addLocalQueryTarget(e,t=!0){return t&&this.Ua.na(e),this.ka[e]||"not-current"}updateQueryState(e,t,r){this.ka[e]=t}removeLocalQueryTarget(e){this.Ua.ra(e)}isLocalQueryTarget(e){return this.Ua.activeTargetIds.has(e)}clearQueryState(e){delete this.ka[e]}getAllActiveQueryTargets(){return this.Ua.activeTargetIds}isActiveQueryTarget(e){return this.Ua.activeTargetIds.has(e)}start(){return this.Ua=new xd,Promise.resolve()}handleUserChange(e,t,r){}setOnlineState(e){}shutdown(){}writeSequenceNumber(e){}notifyBundleLoaded(e){}}function Cl(){return typeof document<"u"?document:null}/**
 * @license
 * Copyright 2018 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class zv{constructor(e,t){this.asyncQueue=e,this.onlineStateHandler=t,this.state="Unknown",this.qa=0,this.$a=null,this.Ka=!0}Wa(){this.qa===0&&(this.Qa("Unknown"),this.$a=this.asyncQueue.enqueueAfterDelay("online_state_timeout",1e4,(()=>(this.$a=null,this.Ga("Backend didn't respond within 10 seconds."),this.Qa("Offline"),Promise.resolve()))))}za(e){this.state==="Online"?this.Qa("Unknown"):(this.qa++,this.qa>=1&&(this.ja(),this.Ga(`Connection failed 1 times. Most recent error: ${e.toString()}`),this.Qa("Offline")))}set(e){this.ja(),this.qa=0,e==="Online"&&(this.Ka=!1),this.Qa(e)}Qa(e){e!==this.state&&(this.state=e,this.onlineStateHandler(e))}Ga(e){const t=`Could not reach Cloud Firestore backend. ${e}
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;this.Ka?(_n(t),this.Ka=!1):W("OnlineStateTracker",t)}ja(){this.$a!==null&&(this.$a.cancel(),this.$a=null)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const rn="RemoteStore";class Wv{constructor(e,t,r,s,i){this.localStore=e,this.datastore=t,this.asyncQueue=r,this.remoteSyncer={},this.Ha=[],this.Ja=new Map,this.Ya=new Map,this.Za=new Map,this.Xa=new rr(1e3),this.eu=new rr(1001),this.tu=new Set,this.nu=[],this.ru=i,this.ru.bt((a=>{r.enqueueAndForget((async()=>{Ur(this)&&(W(rn,"Restarting streams for network reachability change."),await(async function(c){const u=ie(c);u.tu.add(4),await qi(u),u.iu.set("Unknown"),u.tu.delete(4),await Lo(u)})(this))}))})),this.iu=new zv(r,s)}}async function Lo(n){if(Ur(n))for(const e of n.nu)await e(!0)}async function qi(n){for(const e of n.nu)await e(!1)}function sc(n,e){return n.Ya.get(e)||void 0}function sm(n,e){const t=ie(n),r=sc(t,e.targetId);if(r!==void 0&&t.Ja.has(r))return;const s=(function(l,c){const u=sc(l,c);u!==void 0&&l.Za.delete(u);const d=(function(v,w){return w%2!=0?v.eu.next():v.Xa.next()})(l,c);return l.Ya.set(c,d),l.Za.set(d,c),d})(t,e.targetId);W(rn,"remoteStoreListen mapping SDK target ID to remote",e.targetId,s);const i=new dn(e.target,s,e.purpose,e.sequenceNumber,e.snapshotVersion,e.lastLimboFreeSnapshotVersion,e.resumeToken);t.Ja.set(s,i),cu(t)?lu(t):bs(t).Fn()&&ou(t,i)}function au(n,e){const t=ie(n),r=bs(t),s=sc(t,e);W(rn,"remoteStoreUnlisten removing mapping of SDK target ID to remote",e,s),t.Ja.delete(s),t.Ya.delete(e),t.Za.delete(s),r.Fn()&&im(t,s),t.Ja.size===0&&(r.Fn()?r.Nn():Ur(t)&&t.iu.set("Unknown"))}function ou(n,e){if(n.su.We(e.targetId),e.resumeToken.approximateByteSize()>0||e.snapshotVersion.compareTo(se.min())>0){const t=n.Za.get(e.targetId);if(t===void 0)return void W(rn,"SDK target ID not found for remote ID: "+e.targetId);const r=n.remoteSyncer.getRemoteKeysForTarget(t).size;e=e.withExpectedCount(r)}bs(n).jn(e)}function im(n,e){n.su.We(e),bs(n).Hn(e)}function lu(n){n.su=new Y8({getRemoteKeysForTarget:e=>{const t=n.Za.get(e);return t!==void 0?n.remoteSyncer.getRemoteKeysForTarget(t):ce()},dt:e=>n.Ja.get(e)||null,Tt:()=>n.datastore.serializer.databaseId}),bs(n).start(),n.iu.Wa()}function cu(n){return Ur(n)&&!bs(n).Cn()&&n.Ja.size>0}function Ur(n){return ie(n).tu.size===0}function am(n){n.su=void 0}async function Gv(n){n.iu.set("Online")}async function Kv(n){n.Ja.forEach(((e,t)=>{ou(n,e)}))}async function Qv(n,e){am(n),cu(n)?(n.iu.za(e),lu(n)):n.iu.set("Unknown")}async function Yv(n,e,t){if(n.iu.set("Online"),e instanceof y2&&e.state===2&&e.cause)try{await(async function(s,i){const a=i.cause;for(const l of i.targetIds){if(s.Ja.has(l)){const c=s.Za.get(l);c!==void 0&&(await s.remoteSyncer.rejectListen(c,a),s.Ya.delete(c),s.Za.delete(l)),s.Ja.delete(l)}s.su.removeTarget(l)}})(n,e)}catch(r){W(rn,"Failed to remove targets %s: %s ",e.targetIds.join(","),r),await to(n,r)}else if(e instanceof wa?n.su.et(e):e instanceof g2?n.su.ot(e):n.su.rt(e),!t.isEqual(se.min()))try{const r=await rm(n.localStore);t.compareTo(r)>=0&&await(function(i,a){const l=i.su.Rt(a);l.targetChanges.forEach(((u,d)=>{if(u.resumeToken.approximateByteSize()>0){const p=i.Ja.get(d);p&&i.Ja.set(d,p.withResumeToken(u.resumeToken,a))}})),l.targetMismatches.forEach(((u,d)=>{const p=i.Ja.get(u);if(!p)return;i.Ja.set(u,p.withResumeToken(Ve.EMPTY_BYTE_STRING,p.snapshotVersion)),im(i,u);const v=new dn(p.target,u,d,p.sequenceNumber);ou(i,v)}));const c=(function(d,p){const v=new Map;p.targetChanges.forEach(((b,x)=>{const k=d.Za.get(x);k!==void 0&&v.set(k,b)}));let w=new we(ue);return p.targetMismatches.forEach(((b,x)=>{const k=d.Za.get(b);k!==void 0&&(w=w.insert(k,x))})),new Ui(p.snapshotVersion,v,w,p.documentUpdates,p.augmentedDocumentUpdates,p.resolvedLimboDocuments)})(i,l);return i.remoteSyncer.applyRemoteEvent(c)})(n,t)}catch(r){W(rn,"Failed to raise snapshot:",r),await to(n,r)}}async function to(n,e,t){if(!gs(e))throw e;n.tu.add(1),await qi(n),n.iu.set("Offline"),t||(t=()=>rm(n.localStore)),n.asyncQueue.enqueueRetryable((async()=>{W(rn,"Retrying IndexedDB access"),await t(),n.tu.delete(1),await Lo(n)}))}function om(n,e){return e().catch((t=>to(n,t,e)))}async function Vo(n){const e=ie(n),t=sr(e);let r=e.Ha.length>0?e.Ha[e.Ha.length-1].batchId:kc;for(;Xv(e);)try{const s=await qv(e.localStore,r);if(s===null){e.Ha.length===0&&t.Nn();break}r=s.batchId,Jv(e,s)}catch(s){await to(e,s)}lm(e)&&cm(e)}function Xv(n){return Ur(n)&&n.Ha.length<10}function Jv(n,e){n.Ha.push(e);const t=sr(n);t.Fn()&&t.Jn&&t.Yn(e.mutations)}function lm(n){return Ur(n)&&!sr(n).Cn()&&n.Ha.length>0}function cm(n){sr(n).start()}async function Zv(n){sr(n).er()}async function e_(n){const e=sr(n);for(const t of n.Ha)e.Yn(t.mutations)}async function t_(n,e,t){const r=n.Ha.shift(),s=tu.from(r,e,t);await om(n,(()=>n.remoteSyncer.applySuccessfulWrite(s))),await Vo(n)}async function n_(n,e){e&&sr(n).Jn&&await(async function(r,s){if((function(a){return H8(a)&&a!==M.ABORTED})(s.code)){const i=r.Ha.shift();sr(r).Mn(),await om(r,(()=>r.remoteSyncer.rejectFailedWrite(i.batchId,s))),await Vo(r)}})(n,e),lm(n)&&cm(n)}async function Pd(n,e){const t=ie(n);t.asyncQueue.verifyOperationInProgress(),W(rn,"RemoteStore received new credentials");const r=Ur(t);t.tu.add(3),await qi(t),r&&t.iu.set("Unknown"),await t.remoteSyncer.handleCredentialChange(e),t.tu.delete(3),await Lo(t)}async function r_(n,e){const t=ie(n);e?(t.tu.delete(2),await Lo(t)):e||(t.tu.add(2),await qi(t),t.iu.set("Unknown"))}function bs(n){return n._u||(n._u=(function(t,r,s){const i=ie(t);return i.nr(),new Ay(r,i.connection,i.authCredentials,i.appCheckCredentials,i.serializer,s)})(n.datastore,n.asyncQueue,{Qt:Gv.bind(null,n),zt:Kv.bind(null,n),Ht:Qv.bind(null,n),zn:Yv.bind(null,n)}),n.nu.push((async e=>{e?(n._u.Mn(),cu(n)?lu(n):n.iu.set("Unknown")):(await n._u.stop(),am(n))}))),n._u}function sr(n){return n.ou||(n.ou=(function(t,r,s){const i=ie(t);return i.nr(),new Ey(r,i.connection,i.authCredentials,i.appCheckCredentials,i.serializer,s)})(n.datastore,n.asyncQueue,{Qt:()=>Promise.resolve(),zt:Zv.bind(null,n),Ht:n_.bind(null,n),Zn:e_.bind(null,n),Xn:t_.bind(null,n)}),n.nu.push((async e=>{e?(n.ou.Mn(),await Vo(n)):(await n.ou.stop(),n.Ha.length>0&&(W(rn,`Stopping write stream with ${n.Ha.length} pending writes`),n.Ha=[]))}))),n.ou}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class um{constructor(e){this.key=e}}class hm{constructor(e){this.key=e}}class l_{constructor(e,t){this.query=e,this.Ou=t,this.Mu=null,this.hasCachedResults=!1,this.current=!1,this.Nu=ce(),this.mutatedKeys=ce(),this.Lu=Fe(e)?nc(e):Lc(e),this.Bu=new _r(this.Lu)}get Uu(){return this.Ou}ku(e,t){const r=t?t.qu:new kd,s=t?t.Bu:this.Bu;let i=t?t.mutatedKeys:this.mutatedKeys,a=s,l=!1;const[c,u]=this.$u(this.query,s);e.inorderTraversal(((p,v)=>{const w=s.get(p),b=_v(this.query,v)?v:null,x=!!w&&this.mutatedKeys.has(w.key),k=!!b&&(b.hasLocalMutations||this.mutatedKeys.has(b.key)&&b.hasCommittedMutations);let B=!1;w&&b?w.data.isEqual(b.data)?x!==k&&(r.track({type:3,doc:b}),B=!0):this.Ku(w,b)||(r.track({type:2,doc:b}),B=!0,(c&&this.Lu(b,c)>0||u&&this.Lu(b,u)<0)&&(l=!0)):!w&&b?(r.track({type:0,doc:b}),B=!0):w&&!b&&(r.track({type:1,doc:w}),B=!0,(c||u)&&(l=!0)),B&&(b?(a=a.add(b),i=k?i.add(p):i.delete(p)):(a=a.delete(p),i=i.delete(p)))}));const d=this.Wu(this.query);if(d)if(Fe(this.query)){const p=[];a.forEach((b=>p.push(b)));const v=em(this.query,p);let w=new _r(nc(this.query));for(const b of v)w=w.add(b);a.forEach((b=>{w.has(b.key)||(i=i.delete(b.key),r.track({type:1,doc:b}))})),a=w}else{const p=this.Qu(this.query);for(;a.size>d;){const v=p==="F"?a.last():a.first();a=a.delete(v.key),i=i.delete(v.key),r.track({type:1,doc:v})}}return{Bu:a,qu:r,Uo:l,mutatedKeys:i}}Wu(e){var t;return Fe(e)?(t=Rl(e))==null?void 0:t.limit:e.limit||void 0}Qu(e){if(Fe(e)){const t=Rl(e);return t&&t.limit<0?"L":"F"}return e.limitType}$u(e,t){var r;if(Fe(e)){const s=(r=Rl(e))==null?void 0:r.limit;return[t.size===s?t.last():null,null]}return[e.limitType==="F"&&t.size===this.Wu(this.query)?t.last():null,e.limitType==="L"&&t.size===this.Wu(this.query)?t.first():null]}Ku(e,t){return e.hasLocalMutations&&t.hasCommittedMutations&&!t.hasLocalMutations}applyChanges(e,t,r,s){const i=this.Bu;this.Bu=e.Bu,this.mutatedKeys=e.mutatedKeys;const a=e.qu.cu();a.sort(((d,p)=>(function(w,b){const x=k=>{switch(k){case 0:return 1;case 2:case 3:return 2;case 1:return 0;default:return Z(20277,{ft:k})}};return x(w)-x(b)})(d.type,p.type)||this.Lu(d.doc,p.doc))),this.Gu(r),s=s??!1;const l=t&&!s?this.zu():[],c=this.Nu.size===0&&this.current&&!s?1:0,u=c!==this.Mu;return this.Mu=c,a.length!==0||u?{snapshot:new ds(this.query,e.Bu,i,a,e.mutatedKeys,c===0,u,!1,!!r&&r.resumeToken.approximateByteSize()>0),ju:l}:{ju:l}}Ru(e){return this.current&&e==="Offline"?(this.current=!1,this.applyChanges({Bu:this.Bu,qu:new kd,mutatedKeys:this.mutatedKeys,Uo:!1},!1)):{ju:[]}}Hu(e){return!this.Ou.has(e)&&!!this.Bu.has(e)&&!this.Bu.get(e).hasLocalMutations}Gu(e){e&&(e.addedDocuments.forEach((t=>this.Ou=this.Ou.add(t))),e.modifiedDocuments.forEach((t=>{})),e.removedDocuments.forEach((t=>this.Ou=this.Ou.delete(t))),this.current=e.current)}zu(){if(!this.current)return[];const e=this.Nu;this.Nu=ce(),this.Bu.forEach((r=>{this.Hu(r.key)&&(this.Nu=this.Nu.add(r.key))}));const t=[];return e.forEach((r=>{this.Nu.has(r)||t.push(new hm(r))})),this.Nu.forEach((r=>{e.has(r)||t.push(new um(r))})),t}Ju(e){this.Ou=e.Jo,this.Nu=ce();const t=this.ku(e.documents);return this.applyChanges(t,!0)}Yu(){return ds.fromInitialDocuments(this.query,this.Bu,this.mutatedKeys,this.Mu===0,this.hasCachedResults)}}const gu="SyncEngine";class c_{constructor(e,t,r){this.query=e,this.targetId=t,this.view=r}}class u_{constructor(e){this.key=e,this.Zu=!1}}class h_{constructor(e,t,r,s,i,a){this.localStore=e,this.remoteStore=t,this.eventManager=r,this.sharedClientState=s,this.currentUser=i,this.maxConcurrentLimboResolutions=a,this.Xu={},this.ec=new Vr((l=>X2(l)),Do),this.tc=new Map,this.nc=new Set,this.rc=new we(X.comparator),this.sc=new Map,this._c=new nu,this.oc={},this.ac=new Map,this.uc=rr.Cs(),this.onlineState="Unknown",this.cc=void 0}get isPrimaryClient(){return this.cc===!0}}async function d_(n,e,t=!0){const r=ym(n);let s;const i=r.ec.get(e);return i?(r.sharedClientState.addLocalQueryTarget(i.targetId),s=i.view.Yu()):s=await dm(r,e,t,!0),s}async function f_(n,e){const t=ym(n);await dm(t,e,!0,!1)}async function dm(n,e,t,r){const s=await Hv(n.localStore,Fe(e)?e:Zt(e)),i=s.targetId,a=n.sharedClientState.addLocalQueryTarget(i,t);let l;return r&&(l=await p_(n,e,i,a==="current",s.resumeToken)),n.isPrimaryClient&&t&&sm(n.remoteStore,s),l}async function p_(n,e,t,r,s){n.lc=(p,v,w)=>(async function(x,k,B,V){let F=k.view.ku(B);F.Uo&&(F=await Sd(x.localStore,k.query,!1).then((({documents:T})=>k.view.ku(T,F))));const j=V&&V.targetChanges.get(k.targetId),q=V&&V.targetMismatches.get(k.targetId)!=null,K=k.view.applyChanges(F,x.isPrimaryClient,j,q);return Od(x,k.targetId,K.ju),K.snapshot})(n,p,v,w);const i=await Sd(n.localStore,e,!0),a=new l_(e,i.Jo),l=a.ku(i.documents),c=Bi.createSynthesizedTargetChangeForCurrentChange(t,r&&n.onlineState!=="Offline",s),u=a.applyChanges(l,n.isPrimaryClient,c);Od(n,t,u.ju);const d=new c_(e,t,a);return n.ec.set(e,d),n.tc.has(t)?n.tc.get(t).push(e):n.tc.set(t,[e]),u.snapshot}async function m_(n,e,t){const r=ie(n),s=r.ec.get(e),i=r.tc.get(s.targetId);if(i.length>1)return r.tc.set(s.targetId,i.filter((a=>!Do(a,e)))),void r.ec.delete(e);r.isPrimaryClient?(r.sharedClientState.removeLocalQueryTarget(s.targetId),r.sharedClientState.isActiveQueryTarget(s.targetId)||await rc(r.localStore,s.targetId,!1).then((()=>{r.sharedClientState.clearQueryState(s.targetId),t&&au(r.remoteStore,s.targetId),ac(r,s.targetId)})).catch(ms)):(ac(r,s.targetId),await rc(r.localStore,s.targetId,!0))}async function g_(n,e){const t=ie(n),r=t.ec.get(e),s=t.tc.get(r.targetId);t.isPrimaryClient&&s.length===1&&(t.sharedClientState.removeLocalQueryTarget(r.targetId),au(t.remoteStore,r.targetId))}async function y_(n,e,t){const r=I_(n);try{const s=await(function(a,l){const c=ie(a),u=Ae.now(),d=l.reduce(((w,b)=>w.add(b.key)),ce());let p,v;return c.persistence.runTransaction("Locally write mutations","readwrite",(w=>{let b=ft(),x=ce();return c.Qo.getEntries(w,d).next((k=>{b=k,b.forEach(((B,V)=>{V.isValidDocument()||(x=x.add(B))}))})).next((()=>c.localDocuments.getOverlayedDocuments(w,b))).next((k=>{p=k;const B=[];for(const V of l){const F=b8(V,p.get(V.key).overlayedDocument);F!=null&&B.push(new Lr(V.key,F,Kp(F.value.mapValue),Jt.exists(!0)))}return c.mutationQueue.addMutationBatch(w,u,B,l)})).next((k=>{v=k;const B=k.applyToLocalDocumentSet(p,x);return c.documentOverlayCache.saveOverlays(w,k.batchId,B)}))})).then((()=>({batchId:v.batchId,changes:p2(p)})))})(r.localStore,e);r.sharedClientState.addPendingMutation(s.batchId),(function(a,l,c){let u=a.oc[a.currentUser.toKey()];u||(u=new we(ue)),u=u.insert(l,c),a.oc[a.currentUser.toKey()]=u})(r,s.batchId,t),await Hi(r,s.changes),await Vo(r.remoteStore)}catch(s){const i=hu(s,"Failed to persist write");t.reject(i)}}async function fm(n,e){const t=ie(n);try{const r=await Bv(t.localStore,e);e.targetChanges.forEach(((s,i)=>{const a=t.sc.get(i);a&&(Q(s.addedDocuments.size+s.modifiedDocuments.size+s.removedDocuments.size<=1,22616),s.addedDocuments.size>0?a.Zu=!0:s.modifiedDocuments.size>0?Q(a.Zu,14607):s.removedDocuments.size>0&&(Q(a.Zu,42227),a.Zu=!1))})),await Hi(t,r,e)}catch(r){await ms(r)}}function Dd(n,e,t){const r=ie(n);if(r.isPrimaryClient&&t===0||!r.isPrimaryClient&&t===1){const s=[];r.ec.forEach(((i,a)=>{const l=a.view.Ru(e);l.snapshot&&s.push(l.snapshot)})),(function(a,l){const c=ie(a);c.onlineState=l;let u=!1;c.queries.forEach(((d,p)=>{for(const v of p.Eu)v.Ru(l)&&(u=!0)})),u&&pu(c)})(r.eventManager,e),s.length&&r.Xu.zn(s),r.onlineState=e,r.isPrimaryClient&&r.sharedClientState.setOnlineState(e)}}async function v_(n,e,t){const r=ie(n);r.sharedClientState.updateQueryState(e,"rejected",t);const s=r.sc.get(e),i=s&&s.key;if(i){let a=new we(X.comparator);a=a.insert(i,Je.newNoDocument(i,se.min()));const l=ce().add(i),c=new Ui(se.min(),new Map,new we(ue),a,ft(),l);await fm(r,c),r.rc=r.rc.remove(i),r.sc.delete(e),yu(r)}else await rc(r.localStore,e,!1).then((()=>ac(r,e,t))).catch(ms)}async function __(n,e){const t=ie(n),r=e.batch.batchId;try{const s=await Uv(t.localStore,e);mm(t,r,null),pm(t,r),t.sharedClientState.updateMutationState(r,"acknowledged"),await Hi(t,s)}catch(s){await ms(s)}}async function A_(n,e,t){const r=ie(n);try{const s=await(function(a,l){const c=ie(a);return c.persistence.runTransaction("Reject batch","readwrite-primary",(u=>{let d;return c.mutationQueue.lookupMutationBatch(u,l).next((p=>(Q(p!==null,37113),d=p.keys(),c.mutationQueue.removeMutationBatch(u,p)))).next((()=>c.mutationQueue.performConsistencyCheck(u))).next((()=>c.documentOverlayCache.removeOverlaysForBatchId(u,d,l))).next((()=>c.localDocuments.recalculateAndSaveOverlaysForDocumentKeys(u,d))).next((()=>c.localDocuments.getDocuments(u,d)))}))})(r.localStore,e);mm(r,e,t),pm(r,e),r.sharedClientState.updateMutationState(e,"rejected",t),await Hi(r,s)}catch(s){await ms(s)}}function pm(n,e){(n.ac.get(e)||[]).forEach((t=>{t.resolve()})),n.ac.delete(e)}function mm(n,e,t){const r=ie(n);let s=r.oc[r.currentUser.toKey()];if(s){const i=s.get(e);i&&(t?i.reject(t):i.resolve(),s=s.remove(e)),r.oc[r.currentUser.toKey()]=s}}function ac(n,e,t=null){n.sharedClientState.removeLocalQueryTarget(e);for(const r of n.tc.get(e))n.ec.delete(r),t&&n.Xu.Ec(r,t);n.tc.delete(e),n.isPrimaryClient&&n._c.s_(e).forEach((r=>{n._c.containsKey(r)||gm(n,r)}))}function gm(n,e){n.nc.delete(e.path.canonicalString());const t=n.rc.get(e);t!==null&&(au(n.remoteStore,t),n.rc=n.rc.remove(e),n.sc.delete(t),yu(n))}function Od(n,e,t){for(const r of t)r instanceof um?(n._c.addReference(r.key,e),E_(n,r)):r instanceof hm?(W(gu,"Document no longer in limbo: "+r.key),n._c.removeReference(r.key,e),n._c.containsKey(r.key)||gm(n,r.key)):Z(19791,{hc:r})}function E_(n,e){const t=e.key,r=t.path.canonicalString();n.rc.get(t)||n.nc.has(r)||(W(gu,"New document in limbo: "+t),n.nc.add(r),yu(n))}function yu(n){for(;n.nc.size>0&&n.rc.size<n.maxConcurrentLimboResolutions;){const e=n.nc.values().next().value;n.nc.delete(e);const t=new X(me.fromString(e)),r=n.uc.next();n.sc.set(r,new u_(t)),n.rc=n.rc.insert(t,r),sm(n.remoteStore,new dn(Zt(Io(t.path)),r,"TargetPurposeLimboResolution",vo.ce))}}async function Hi(n,e,t){const r=ie(n),s=[],i=[],a=[];r.ec.isEmpty()||(r.ec.forEach(((l,c)=>{a.push(r.lc(c,e,t).then((u=>{var d;if((u||t)&&r.isPrimaryClient){const p=u?!u.fromCache:(d=t==null?void 0:t.targetChanges.get(c.targetId))==null?void 0:d.current;r.sharedClientState.updateQueryState(c.targetId,p?"current":"not-current")}if(u){s.push(u);const p=su.vo(c.targetId,u);i.push(p)}})))})),await Promise.all(a),r.Xu.zn(s),await(async function(c,u){const d=ie(c);try{await d.persistence.runTransaction("notifyLocalViewChanges","readwrite",(p=>U.forEach(u,(v=>U.forEach(v.wo,(w=>d.persistence.referenceDelegate.addReference(p,v.targetId,w))).next((()=>U.forEach(v.bo,(w=>d.persistence.referenceDelegate.removeReference(p,v.targetId,w)))))))))}catch(p){if(!gs(p))throw p;W(iu,"Failed to update sequence numbers: "+p)}for(const p of u){const v=p.targetId;if(!p.fromCache){const w=d.$o.get(v),b=w.snapshotVersion,x=w.withLastLimboFreeSnapshotVersion(b);d.$o=d.$o.insert(v,x)}}})(r.localStore,i))}async function w_(n,e){const t=ie(n);if(!t.currentUser.isEqual(e)){W(gu,"User change. New user:",e.toKey());const r=await nm(t.localStore,e);t.currentUser=e,(function(i,a){i.ac.forEach((l=>{l.forEach((c=>{c.reject(new z(M.CANCELLED,a))}))})),i.ac.clear()})(t,"'waitForPendingWrites' promise is rejected due to a user change."),t.sharedClientState.handleUserChange(e,r.removedBatchIds,r.addedBatchIds),await Hi(t,r.zo)}}function b_(n,e){const t=ie(n),r=t.sc.get(e);if(r&&r.Zu)return ce().add(r.key);{let s=ce();const i=t.tc.get(e);if(!i)return s;for(const a of i??[]){const l=t.ec.get(a);s=s.unionWith(l.view.Uu)}return s}}function ym(n){const e=ie(n);return e.remoteStore.remoteSyncer.applyRemoteEvent=fm.bind(null,e),e.remoteStore.remoteSyncer.getRemoteKeysForTarget=b_.bind(null,e),e.remoteStore.remoteSyncer.rejectListen=v_.bind(null,e),e.Xu.zn=a_.bind(null,e.eventManager),e.Xu.Ec=o_.bind(null,e.eventManager),e}function I_(n){const e=ie(n);return e.remoteStore.remoteSyncer.applySuccessfulWrite=__.bind(null,e),e.remoteStore.remoteSyncer.rejectFailedWrite=A_.bind(null,e),e}class no{constructor(){this.kind="memory",this.synchronizeTabs=!1}async initialize(e){this.serializer=Ro(e.databaseInfo.databaseId),this.sharedClientState=this.Rc(e),this.persistence=this.Ic(e),await this.persistence.start(),this.localStore=this.Ac(e),this.gcScheduler=this.Vc(e,this.localStore),this.indexBackfillerScheduler=this.dc(e,this.localStore)}Vc(e,t){return null}dc(e,t){return null}Ac(e){return Mv(this.persistence,new Ov,e.initialUser,this.serializer)}Ic(e){return new tm(ru.C_,this.serializer)}Rc(e){return new $v}async terminate(){var e,t;(e=this.gcScheduler)==null||e.stop(),(t=this.indexBackfillerScheduler)==null||t.stop(),this.sharedClientState.shutdown(),await this.persistence.shutdown()}}no.provider={build:()=>new no};class T_ extends no{constructor(e){super(),this.cacheSizeBytes=e}Vc(e,t){Q(this.persistence.referenceDelegate instanceof eo,46915);const r=this.persistence.referenceDelegate.garbageCollector;return new xy(r,e.asyncQueue,t)}Ic(e){const t=this.cacheSizeBytes!==void 0?dt.withCacheSize(this.cacheSizeBytes):dt.DEFAULT;return new tm((r=>eo.C_(r,t)),this.serializer)}}class oc{async initialize(e,t){this.localStore||(this.localStore=e.localStore,this.sharedClientState=e.sharedClientState,this.datastore=this.createDatastore(t),this.remoteStore=this.createRemoteStore(t),this.eventManager=this.createEventManager(t),this.syncEngine=this.createSyncEngine(t,!e.synchronizeTabs),this.sharedClientState.onlineStateHandler=r=>Dd(this.syncEngine,r,1),this.remoteStore.remoteSyncer.handleCredentialChange=w_.bind(null,this.syncEngine),await r_(this.remoteStore,this.syncEngine.isPrimaryClient))}createEventManager(e){return(function(){return new i_})()}createDatastore(e){const t=Ro(e.databaseInfo.databaseId),r=_y(e.databaseInfo);return Iy(e.authCredentials,e.appCheckCredentials,r,t)}createRemoteStore(e){return(function(r,s,i,a,l){return new Wv(r,s,i,a,l)})(this.localStore,this.datastore,e.asyncQueue,(t=>Dd(this.syncEngine,t,0)),(function(){return vd.C()?new vd:new my})())}createSyncEngine(e,t){return(function(s,i,a,l,c,u,d){const p=new h_(s,i,a,l,c,u);return d&&(p.cc=!0),p})(this.localStore,this.remoteStore,this.eventManager,this.sharedClientState,e.initialUser,e.maxConcurrentLimboResolutions,t)}async terminate(){var e,t;await(async function(s){const i=ie(s);W(rn,"RemoteStore shutting down."),i.tu.add(5),await qi(i),i.ru.shutdown(),i.iu.set("Unknown")})(this.remoteStore),(e=this.datastore)==null||e.terminate(),(t=this.eventManager)==null||t.terminate()}}oc.provider={build:()=>new oc};/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class vu{constructor(e){this.observer=e,this.muted=!1}next(e){this.muted||this.observer.next&&this.mc(this.observer.next,e)}error(e){this.muted||(this.observer.error?this.mc(this.observer.error,e):_n("Uncaught Error in snapshot listener:",e.toString()))}gc(){this.muted=!0}mc(e,t){setTimeout((()=>{this.muted||e(t)}),0)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class _u extends L_{constructor(e){super(),this.firestore=e}convertBytes(e){return new Pt(e)}convertReference(e){const t=this.convertDocumentKey(e,this.firestore._databaseId);return new Ce(this.firestore,null,t)}}const Bd="@firebase/firestore",Fd="4.16.0";/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class _m{constructor(e,t,r,s,i){this._firestore=e,this._userDataWriter=t,this._key=r,this._document=s,this._converter=i}get id(){return this._key.path.lastSegment()}get ref(){return new Ce(this._firestore,this._converter,this._key)}exists(){return this._document!==null}data(){if(this._document){if(this._converter){const e=new V_(this._firestore,this._userDataWriter,this._key,this._document,null);return this._converter.fromFirestore(e)}return this._userDataWriter.convertValue(this._document.data.value)}}_fieldsProto(){var e;return((e=this._document)==null?void 0:e.data.clone().value.mapValue.fields)??void 0}get(e){if(this._document){const t=this._document.data.field(vs("DocumentSnapshot.get",e));if(t!==null)return this._userDataWriter.convertValue(t)}}}class V_ extends _m{data(){return super.data()}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function ss(n){n=Bt(n,Ce);const e=Bt(n.firestore,xr),t=Mo(e);return P_(t,n._key).then((r=>Im(e,n,r)))}function xl(n){n=Bt(n,bn);const e=Bt(n.firestore,xr),t=Mo(e),r=new _u(e);return Am(n._query),k_(t,n._query).then((s=>new Er(e,r,n,s)))}function U_(n,e,t){n=Bt(n,Ce);const r=Bt(n.firestore,xr),s=wm(n.converter,e,t),i=Hc(r);return bm(r,[D2(i,"setDoc",n._key,s,n.converter!==null,t).toMutation(n._key,Jt.none())])}function Sa(n,e){const t=Bt(n.firestore,xr),r=Wn(n),s=wm(n.converter,e),i=Hc(n.firestore);return bm(t,[D2(i,"addDoc",r._key,s,n.converter!==null,{}).toMutation(r._key,Jt.exists(!1))]).then((()=>r))}function B_(n,...e){var u,d,p;n=Te(n);let t={includeMetadataChanges:!1,source:"default"},r=0;typeof e[r]!="object"||qd(e[r])||(t=e[r++]);const s={includeMetadataChanges:t.includeMetadataChanges,source:t.source};if(qd(e[r])){const v=e[r];e[r]=(u=v.next)==null?void 0:u.bind(v),e[r+1]=(d=v.error)==null?void 0:d.bind(v),e[r+2]=(p=v.complete)==null?void 0:p.bind(v)}let i,a,l;if(n instanceof Ce)a=Bt(n.firestore,xr),l=Io(n._key.path),i={next:v=>{e[r]&&e[r](Im(a,n,v))},error:e[r+1],complete:e[r+2]};else{const v=Bt(n,bn);a=Bt(v.firestore,xr),l=v._query;const w=new _u(a);i={next:b=>{e[r]&&e[r](new Er(a,w,v,b))},error:e[r+1],complete:e[r+2]},Am(n._query)}const c=Mo(a);return x_(c,l,s,i)}function bm(n,e){const t=Mo(n);return N_(t,e)}function Im(n,e,t){const r=t.docs.get(e._key),s=new _u(n);return new Ar(n,s,e._key,r,new Qs(t.hasPendingWrites,t.fromCache),e.converter)}(function(e,t=!0){j9(Dr),Ft(new Dt("firestore",((r,{instanceIdentifier:s,options:i})=>{const a=r.getProvider("app").getImmediate(),l=new xr(new W9(r.getProvider("auth-internal")),new Q9(a,r.getProvider("app-check-internal")),p8(a,s),a);return i={useFetchStreams:t,...i},l._setSettings(i),l}),"PUBLIC").setMultipleInstances(!0)),pt(Bd,Fd,e),pt(Bd,Fd,"esm2020")})();/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Tm="firebasestorage.googleapis.com",Rm="storageBucket",F_=120*1e3,q_=600*1e3;/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class xe extends Lt{constructor(e,t,r=0){super(Pl(e),`Firebase Storage: ${t} (${Pl(e)})`),this.status_=r,this.customData={serverResponse:null},this._baseMessage=this.message,Object.setPrototypeOf(this,xe.prototype)}get status(){return this.status_}set status(e){this.status_=e}_codeEquals(e){return Pl(e)===this.code}get serverResponse(){return this.customData.serverResponse}set serverResponse(e){this.customData.serverResponse=e,this.customData.serverResponse?this.message=`${this._baseMessage}
${this.customData.serverResponse}`:this.message=this._baseMessage}}var Se;(function(n){n.UNKNOWN="unknown",n.OBJECT_NOT_FOUND="object-not-found",n.BUCKET_NOT_FOUND="bucket-not-found",n.PROJECT_NOT_FOUND="project-not-found",n.QUOTA_EXCEEDED="quota-exceeded",n.UNAUTHENTICATED="unauthenticated",n.UNAUTHORIZED="unauthorized",n.UNAUTHORIZED_APP="unauthorized-app",n.RETRY_LIMIT_EXCEEDED="retry-limit-exceeded",n.INVALID_CHECKSUM="invalid-checksum",n.CANCELED="canceled",n.INVALID_EVENT_NAME="invalid-event-name",n.INVALID_URL="invalid-url",n.INVALID_DEFAULT_BUCKET="invalid-default-bucket",n.NO_DEFAULT_BUCKET="no-default-bucket",n.CANNOT_SLICE_BLOB="cannot-slice-blob",n.SERVER_FILE_WRONG_SIZE="server-file-wrong-size",n.NO_DOWNLOAD_URL="no-download-url",n.INVALID_ARGUMENT="invalid-argument",n.INVALID_ARGUMENT_COUNT="invalid-argument-count",n.APP_DELETED="app-deleted",n.INVALID_ROOT_OPERATION="invalid-root-operation",n.INVALID_FORMAT="invalid-format",n.INTERNAL_ERROR="internal-error",n.UNSUPPORTED_ENVIRONMENT="unsupported-environment"})(Se||(Se={}));function Pl(n){return"storage/"+n}function Tu(){const n="An unknown error occurred, please check the error payload for server response.";return new xe(Se.UNKNOWN,n)}function H_(n){return new xe(Se.OBJECT_NOT_FOUND,"Object '"+n+"' does not exist.")}function j_(n){return new xe(Se.QUOTA_EXCEEDED,"Quota for bucket '"+n+"' exceeded, please view quota on https://firebase.google.com/pricing/.")}function $_(){const n="User is not authenticated, please authenticate using Firebase Authentication and try again.";return new xe(Se.UNAUTHENTICATED,n)}function z_(){return new xe(Se.UNAUTHORIZED_APP,"This app does not have permission to access Firebase Storage on this project.")}function W_(n){return new xe(Se.UNAUTHORIZED,"User does not have permission to access '"+n+"'.")}function G_(){return new xe(Se.RETRY_LIMIT_EXCEEDED,"Max retry time for operation exceeded, please try again.")}function K_(){return new xe(Se.CANCELED,"User canceled the upload/download.")}function Q_(n){return new xe(Se.INVALID_URL,"Invalid URL '"+n+"'.")}function Y_(n){return new xe(Se.INVALID_DEFAULT_BUCKET,"Invalid default bucket '"+n+"'.")}function X_(){return new xe(Se.NO_DEFAULT_BUCKET,"No default bucket found. Did you set the '"+Rm+"' property when initializing the app?")}function J_(){return new xe(Se.CANNOT_SLICE_BLOB,"Cannot slice blob for upload. Please retry the upload.")}function Z_(){return new xe(Se.NO_DOWNLOAD_URL,"The given file does not have any download URLs.")}function eA(n){return new xe(Se.UNSUPPORTED_ENVIRONMENT,`${n} is missing. Make sure to install the required polyfills. See https://firebase.google.com/docs/web/environments-js-sdk#polyfills for more information.`)}function lc(n){return new xe(Se.INVALID_ARGUMENT,n)}function Cm(){return new xe(Se.APP_DELETED,"The Firebase app was deleted.")}function tA(n){return new xe(Se.INVALID_ROOT_OPERATION,"The operation '"+n+"' cannot be performed on a root reference, create a non-root reference using child, such as .child('file.png').")}function ai(n,e){return new xe(Se.INVALID_FORMAT,"String does not match format '"+n+"': "+e)}function Hs(n){throw new xe(Se.INTERNAL_ERROR,"Internal error: "+n)}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Ct{constructor(e,t){this.bucket=e,this.path_=t}get path(){return this.path_}get isRoot(){return this.path.length===0}fullServerUrl(){const e=encodeURIComponent;return"/b/"+e(this.bucket)+"/o/"+e(this.path)}bucketOnlyServerUrl(){return"/b/"+encodeURIComponent(this.bucket)+"/o"}static makeFromBucketSpec(e,t){let r;try{r=Ct.makeFromUrl(e,t)}catch{return new Ct(e,"")}if(r.path==="")return r;throw Y_(e)}static makeFromUrl(e,t){let r=null;const s="([A-Za-z0-9.\\-_]+)";function i(j){j.path.charAt(j.path.length-1)==="/"&&(j.path_=j.path_.slice(0,-1))}const a="(/(.*))?$",l=new RegExp("^gs://"+s+a,"i"),c={bucket:1,path:3};function u(j){j.path_=decodeURIComponent(j.path)}const d="v[A-Za-z0-9_]+",p=t.replace(/[.]/g,"\\."),v="(/([^?#]*).*)?$",w=new RegExp(`^https?://${p}/${d}/b/${s}/o${v}`,"i"),b={bucket:1,path:3},x=t===Tm?"(?:storage.googleapis.com|storage.cloud.google.com)":t,k="([^?#]*)",B=new RegExp(`^https?://${x}/${s}/${k}`,"i"),F=[{regex:l,indices:c,postModify:i},{regex:w,indices:b,postModify:u},{regex:B,indices:{bucket:1,path:2},postModify:u}];for(let j=0;j<F.length;j++){const q=F[j],K=q.regex.exec(e);if(K){const T=K[q.indices.bucket];let g=K[q.indices.path];g||(g=""),r=new Ct(T,g),q.postModify(r);break}}if(r==null)throw Q_(e);return r}}class nA{constructor(e){this.promise_=Promise.reject(e)}getPromise(){return this.promise_}cancel(e=!1){}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function rA(n,e,t){let r=1,s=null,i=null,a=!1,l=0;function c(){return l===2}let u=!1;function d(...k){u||(u=!0,e.apply(null,k))}function p(k){s=setTimeout(()=>{s=null,n(w,c())},k)}function v(){i&&clearTimeout(i)}function w(k,...B){if(u){v();return}if(k){v(),d.call(null,k,...B);return}if(c()||a){v(),d.call(null,k,...B);return}r<64&&(r*=2);let F;l===1?(l=2,F=0):F=(r+Math.random())*1e3,p(F)}let b=!1;function x(k){b||(b=!0,v(),!u&&(s!==null?(k||(l=2),clearTimeout(s),p(0)):k||(l=1)))}return p(0),i=setTimeout(()=>{a=!0,x(!0)},t),x}function sA(n){n(!1)}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Qt={RAW:"raw",BASE64:"base64",BASE64URL:"base64url",DATA_URL:"data_url"};class kl{constructor(e,t){this.data=e,this.contentType=t||null}}function vA(n,e){switch(n){case Qt.RAW:return new kl(xm(e));case Qt.BASE64:case Qt.BASE64URL:return new kl(Pm(n,e));case Qt.DATA_URL:return new kl(AA(e),EA(e))}throw Tu()}function xm(n){const e=[];for(let t=0;t<n.length;t++){let r=n.charCodeAt(t);if(r<=127)e.push(r);else if(r<=2047)e.push(192|r>>6,128|r&63);else if((r&64512)===55296)if(!(t<n.length-1&&(n.charCodeAt(t+1)&64512)===56320))e.push(239,191,189);else{const i=r,a=n.charCodeAt(++t);r=65536|(i&1023)<<10|a&1023,e.push(240|r>>18,128|r>>12&63,128|r>>6&63,128|r&63)}else(r&64512)===56320?e.push(239,191,189):e.push(224|r>>12,128|r>>6&63,128|r&63)}return new Uint8Array(e)}function _A(n){let e;try{e=decodeURIComponent(n)}catch{throw ai(Qt.DATA_URL,"Malformed data URL.")}return xm(e)}function Pm(n,e){switch(n){case Qt.BASE64:{const s=e.indexOf("-")!==-1,i=e.indexOf("_")!==-1;if(s||i)throw ai(n,"Invalid character '"+(s?"-":"_")+"' found: is it base64url encoded?");break}case Qt.BASE64URL:{const s=e.indexOf("+")!==-1,i=e.indexOf("/")!==-1;if(s||i)throw ai(n,"Invalid character '"+(s?"+":"/")+"' found: is it base64 encoded?");e=e.replace(/-/g,"+").replace(/_/g,"/");break}}let t;try{t=yA(e)}catch(s){throw s.message.includes("polyfill")?s:ai(n,"Invalid character found")}const r=new Uint8Array(t.length);for(let s=0;s<t.length;s++)r[s]=t.charCodeAt(s);return r}class km{constructor(e){this.base64=!1,this.contentType=null;const t=e.match(/^data:([^,]+)?,/);if(t===null)throw ai(Qt.DATA_URL,"Must be formatted 'data:[<mediatype>][;base64],<data>");const r=t[1]||null;r!=null&&(this.base64=wA(r,";base64"),this.contentType=this.base64?r.substring(0,r.length-7):r),this.rest=e.substring(e.indexOf(",")+1)}}function AA(n){const e=new km(n);return e.base64?Pm(Qt.BASE64,e.rest):_A(e.rest)}function EA(n){return new km(n).contentType}function wA(n,e){return n.length>=e.length?n.substring(n.length-e.length)===e:!1}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Mm(n){if(!n)throw Tu()}function kA(n,e){function t(r,s){const i=Lm(n,s,e);return Mm(i!==null),i}return t}function NA(n,e){function t(r,s){const i=Lm(n,s,e);return Mm(i!==null),xA(i,s,n.host,n._protocol)}return t}function Um(n){function e(t,r){let s;return t.getStatus()===401?t.getErrorText().includes("Firebase App Check token is invalid")?s=z_():s=$_():t.getStatus()===402?s=j_(n.bucket):t.getStatus()===403?s=W_(n.path):s=r,s.status=t.getStatus(),s.serverResponse=r.serverResponse,s}return e}function DA(n){const e=Um(n);function t(r,s){let i=e(r,s);return r.getStatus()===404&&(i=H_(n.path)),i.serverResponse=s.serverResponse,i}return t}function OA(n,e,t){const r=e.fullServerUrl(),s=Su(r,n.host,n._protocol),i="GET",a=n.maxOperationRetryTime,l=new Vm(s,i,NA(n,t),a);return l.errorHandler=DA(e),l}function LA(n,e){return n&&n.contentType||e&&e.type()||"application/octet-stream"}function VA(n,e,t){const r=Object.assign({},t);return r.fullPath=n.path,r.size=e.size(),r.contentType||(r.contentType=LA(null,e)),r}function MA(n,e,t,r,s){const i=e.bucketOnlyServerUrl(),a={"X-Goog-Upload-Protocol":"multipart"};function l(){let F="";for(let j=0;j<2;j++)F=F+Math.random().toString().slice(2);return F}const c=l();a["Content-Type"]="multipart/related; boundary="+c;const u=VA(e,r,s),d=PA(u,t),p="--"+c+`\r
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Pr{constructor(e,t){this._service=e,t instanceof Ct?this._location=t:this._location=Ct.makeFromUrl(t,e.host)}toString(){return"gs://"+this._location.bucket+"/"+this._location.path}_newRef(e,t){return new Pr(e,t)}get root(){const e=new Ct(this._location.bucket,"");return this._newRef(this._service,e)}get bucket(){return this._location.bucket}get fullPath(){return this._location.path}get name(){return Dm(this._location.path)}get storage(){return this._service}get parent(){const e=bA(this._location.path);if(e===null)return null;const t=new Ct(this._location.bucket,e);return new Pr(this._service,t)}_throwIfRoot(e){if(this._location.path==="")throw tA(e)}}function FA(n,e,t){n._throwIfRoot("uploadBytes");const r=MA(n.storage,n._location,Om(),new Fn(e,!0),t);return n.storage.makeRequestWithTokens(r,Bm).then(s=>({metadata:s,ref:n}))}function qA(n){n._throwIfRoot("getDownloadURL");const e=OA(n.storage,n._location,Om());return n.storage.makeRequestWithTokens(e,Bm).then(t=>{if(t===null)throw Z_();return t})}function HA(n,e){const t=IA(n._location.path,e),r=new Ct(n._location.bucket,t);return new Pr(n.storage,r)}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function jA(n){return/^[A-Za-z]+:\/\//.test(n)}function $A(n,e){return new Pr(n,e)}function Fm(n,e){if(n instanceof xu){const t=n;if(t._bucket==null)throw X_();const r=new Pr(t,t._bucket);return e!=null?Fm(r,e):r}else return e!==void 0?HA(n,e):n}function zA(n,e){if(e&&jA(e)){if(n instanceof xu)return $A(n,e);throw lc("To use ref(service, url), the first argument must be a Storage instance.")}else return Fm(n,e)}function Kd(n,e){const t=e==null?void 0:e[Rm];return t==null?null:Ct.makeFromBucketSpec(t,n)}function WA(n,e,t,r={}){n.host=`${e}:${t}`;const s=Nr(e);s&&fc(`https://${n.host}/b`),n._isUsingEmulator=!0,n._protocol=s?"https":"http";const{mockUserToken:i}=r;i&&(n._overrideAuthToken=typeof i=="string"?i:_f(i,n.app.options.projectId))}class xu{constructor(e,t,r,s,i,a=!1){this.app=e,this._authProvider=t,this._appCheckProvider=r,this._url=s,this._firebaseVersion=i,this._isUsingEmulator=a,this._bucket=null,this._host=Tm,this._protocol="https",this._appId=null,this._deleted=!1,this._maxOperationRetryTime=F_,this._maxUploadRetryTime=q_,this._requests=new Set,s!=null?this._bucket=Ct.makeFromBucketSpec(s,this._host):this._bucket=Kd(this._host,this.app.options)}get host(){return this._host}set host(e){this._host=e,this._url!=null?this._bucket=Ct.makeFromBucketSpec(this._url,e):this._bucket=Kd(e,this.app.options)}get maxUploadRetryTime(){return this._maxUploadRetryTime}set maxUploadRetryTime(e){Gd("time",0,Number.POSITIVE_INFINITY,e),this._maxUploadRetryTime=e}get maxOperationRetryTime(){return this._maxOperationRetryTime}set maxOperationRetryTime(e){Gd("time",0,Number.POSITIVE_INFINITY,e),this._maxOperationRetryTime=e}async _getAuthToken(){if(this._overrideAuthToken)return this._overrideAuthToken;const e=this._authProvider.getImmediate({optional:!0});if(e){const t=await e.getToken();if(t!==null)return t.accessToken}return null}async _getAppCheckToken(){if(It(this.app)&&this.app.settings.appCheckToken)return this.app.settings.appCheckToken;const e=this._appCheckProvider.getImmediate({optional:!0});return e?(await e.getToken()).token:null}_delete(){return this._deleted||(this._deleted=!0,this._requests.forEach(e=>e.cancel()),this._requests.clear()),Promise.resolve()}_makeStorageReference(e){return new Pr(this,e)}_makeRequest(e,t,r,s,i=!0){if(this._deleted)return new nA(Cm());{const a=fA(e,this._appId,r,s,t,this._firebaseVersion,i,this._isUsingEmulator);return this._requests.add(a),a.getPromise().then(()=>this._requests.delete(a),()=>this._requests.delete(a)),a}}async makeRequestWithTokens(e,t){const[r,s]=await Promise.all([this._getAuthToken(),this._getAppCheckToken()]);return this._makeRequest(e,t,r,s).getPromise()}}const Qd="@firebase/storage",Yd="0.14.3";/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
